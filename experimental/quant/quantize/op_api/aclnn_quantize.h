 ...
 * @param [in] zeroPoints: npu
 * device侧的aclTensor，数据类型支持INT32、INT8、UINT8、BFLOAT16，数据格式支持ND，支持传入空指针。
 * @param [in] dtype: 量化输出的数据类型，aclDataType类型，支持ACL_INT8、ACL_UINT8、ACL_INT32。
 * @param [in] axis: Host侧的整型，需要进行量化的轴，取值范围为[-x的维度数, x的维度数-1]。
