...
- 接口功能：对输入张量x进行量化处理，将浮点输入按`scales`、`zeroPoints`量化为`dtype`指定的定点类型。`scales`的size为1时为per-tensor量化；size大于1时为per-channel量化，`scales`、`zeroPoints`与`x`中`axis`指定轴的元素一一对应，其余轴做broadcast。
- 计算公式：
  ...
  其中round为就近舍入（.5舍入到偶数），舍入结果超出`dtype`表示范围时做饱和处理。

## 函数原型

每个算子分为[两段式接口](../../../../docs/zh/context/two_phase_api.md)，必须先调用“aclnnQuantizeGetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnQuantize”接口执行计算。
...
- **参数说明**
  ...
  <tbody>
    ...
    <tr>
      ...
      <td><ul><li>支持空Tensor。</li><li>size需要为1或和输入x中axis轴的size相等。</li><li>数据类型为BFLOAT16时，`x`的数据类型也需要为BFLOAT16。</li></ul></td>
      <td>FLOAT32、FLOAT16、BFLOAT16</td>
      <td>ND</td>
      <td>1</td>
      <td>√</td>
    ...
    <tr>
      ...
      <td><ul><li>支持空Tensor。</li><li>支持传入空指针，传入空指针时按零点为0处理。</li><li>size需要为1或和输入x中axis轴的size相等，并与scales的size相等。</li><li>数据类型为BFLOAT16时，`x`、`scales`的数据类型也需要为BFLOAT16。</li></ul></td>
      <td>INT32、INT8、UINT8、BFLOAT16</td>
      <td>ND</td>
      <td>1</td>
      <td>√</td>
    </tr>
    <tr>
      ...
      <td>输入</td>
      <td>指定输出的数据类型。</td>
      <td>支持配置为ACL_INT8、ACL_UINT8、ACL_INT32。</td>
      <td>-</td>
      <td>-</td>
      <td>-</td>
      <td>-</td>
    ...
    <tr>
      ...
      <td><ul><li>当输入的scales和zeroPoints的size均为1时，该参数实际不使用。</li><li>取值范围为[-x的维度数, x的维度数-1]，负值表示从后往前计数的轴。</li></ul></td>
      <td>-</td>
      <td>-</td>
      <td>-</td>
      <td>-</td>
...
- **返回值**
  ...
  <tbody>
    ...
    <tr>
      <td>输出out的shape与输入x的shape不一致。</td>
    </tr>
    <tr>
      <td>x、scales、zeroPoints的数据类型不满足BFLOAT16同时生效的组合要求。</td>
    </tr>
  </tbody></table>
...
- 数据类型组合：`scales`或`zeroPoints`的数据类型为BFLOAT16时，`x`、`scales`以及`zeroPoints`（若传入）的数据类型必须同时为BFLOAT16；其余场景下`x`与`scales`的浮点类型可自由组合。
- 量化模式：`scales`、`zeroPoints`的size为1时按per-tensor量化，此时`axis`不生效；size大于1时按per-channel量化，size需与`x`中`axis`轴的size相等。
- 空Tensor：`x`、`scales`、`zeroPoints`、`out`中存在空Tensor时，接口返回成功且不执行计算，`out`的内容保持不变。
- `scales`中值为0的元素按缩放系数1.0处理，不会产生inf或nan。
- 确定性计算：
