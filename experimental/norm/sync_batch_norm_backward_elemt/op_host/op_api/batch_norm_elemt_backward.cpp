...
const aclTensor* SyncBatchNormBackwardElemt(const aclTensor* gradOut, const aclTensor* input, const aclTensor* mean,
                                            const aclTensor* invstd, const aclTensor* weight, const aclTensor* meanDy,
                                            const aclTensor* meanDyXmu, aclOpExecutor* executor)
{
    L0_DFX(SyncBatchNormBackwardElemt, gradOut, input, mean, invstd, weight, meanDy, meanDyXmu);
    ...
    auto ret = ADD_TO_LAUNCHER_LIST_AICORE(SyncBatchNormBackwardElemt,
                                           OP_INPUT(gradOut, input, mean, invstd, weight, meanDy, meanDyXmu),
                                           OP_OUTPUT(gradInput));
