 ...
 */
__aicore__ inline void ReduceSumForSmallReduceDim(const LocalTensor<float>& dstLocal3,
                                                  const LocalTensor<float>& srcLocal,
                                                  ...
                                                  const uint32_t repeat, const uint8_t repStride)
{
    ...
    if (repeatTimes == 0) {
        ReduceSumForSmallReduceDimPreRepeat(dstLocal3, srcLocal, tmpLocal, ELEM_PER_REP_FP32, numLastDim, tailCount,
                                            repeat, repStride);
    } else {
        ...
        for (; repIndex + MAX_REP_NUM <= repeat; repIndex += MAX_REP_NUM) {
            ReduceSumForSmallReduceDimPreRepeat(dstLocal3[repIndex], srcLocal[repIndex * numLastDimAligned],
...
__aicore__ inline void ReduceSumMultiN(const LocalTensor<float>& dstLocal, const LocalTensor<float>& srcLocal,
                                       const LocalTensor<float>& tmpLocal3, const uint32_t numRow,
...
#endif // REDUCE_COMMON_H_RMS_NORM
