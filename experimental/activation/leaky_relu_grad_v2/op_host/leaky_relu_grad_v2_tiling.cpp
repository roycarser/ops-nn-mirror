...
#include "../op_kernel/leaky_relu_grad_v2_tiling_data.h"
#include "../op_kernel/leaky_relu_grad_v2_tiling_key.h"
...
struct LeakyReluGradV2CompileInfo {};
...
static ge::graphStatus GetShapeAttrsInfo(gert::TilingContext* context, int64_t& totalIdx, ge::DataType& dataType)
{
    ...
    // 校验维度数一致
    if (inputShapeX.GetDimNum() != outShapeZ.GetDimNum() || inputShapeX.GetDimNum() != inputShapeY.GetDimNum()) {
        shapeMatch = false;
    } else {
        // 校验每个维度的大小一致
        size_t dimNum = inputShapeX.GetDimNum();
        for (size_t i = 0; i < dimNum; i++) {
            if (inputShapeX.GetDim(i) != outShapeZ.GetDim(i) || inputShapeX.GetDim(i) != inputShapeY.GetDim(i)) {
                shapeMatch = false;
                break;
            }
        }
...
// tiling 分发入口
// 可直接替换你的 LeakyReluGradV2TilingFunc 内部实现（保留函数签名）
static ge::graphStatus LeakyReluGradV2TilingFunc(gert::TilingContext* context)
{
    // 1. platform
    uint64_t ubSize = 0;
    int64_t coreNum = 0;
    ...
    if (totalIdx <= 0) {
        ...
        memset_s(tiling, sizeof(LeakyReluGradV2TilingData), 0, sizeof(LeakyReluGradV2TilingData));
    ...
    LeakyReluGradV2TilingData* tiling = context->GetTilingData<LeakyReluGradV2TilingData>();
    OP_CHECK_NULL_WITH_CONTEXT(context, tiling);
    OP_CHECK_IF(memset_s(tiling, sizeof(LeakyReluGradV2TilingData), 0, sizeof(LeakyReluGradV2TilingData)) != EOK,
    ...
    uint32_t ubDataNumber = (inputBytes == 1ULL) ? 7U : 7U;
