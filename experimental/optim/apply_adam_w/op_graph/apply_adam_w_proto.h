 ...
 * \brief
 */
#ifndef OPS_BUILT_IN_OP_PROTO_INC_APPLY_ADAM_W_PROTO_H_
#define OPS_BUILT_IN_OP_PROTO_INC_APPLY_ADAM_W_PROTO_H_

#include "graph/operator_reg.h"
namespace ge {
/**
 * @brief Updates "var" according to the AdamW algorithm. For clarity, the output variable is suffixed with "_out" in
 * the fomulas below (e.g. m_out). Note that beta1_power_out, beta2_power_out, max_grad_norm_out and var_t shown in the
 * formulas are internal intermediate values used to describe the calculation procedure, they are NOT operator outputs;
 * this operator only outputs var, m and v.
 * @code{.c}
 *  if maximize:
 *       gt = -grad
 *   else:
 ...
 * @li weight_decay: Specifying weight decay. A scalar of the same type as "var", value is between 0 and 1 generally
 * (e.g. 1e-2).
 ...
 *
 * @par Attributes:
 * @li amsgrad: An optional bool. Specifying using max_grad_norm in the calculation. Defaults to "False", only support
 * "False".
 * @li maximize: An optional bool. Specifying maximize the objective with respect to the params, instead of minimizing.
 * Defaults to "False".
 *
 * @par Outputs:
