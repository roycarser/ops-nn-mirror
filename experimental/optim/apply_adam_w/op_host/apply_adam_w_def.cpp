...
public:
    explicit ApplyAdamW(const char* name) : OpDef(name)
    {
        ...
        this->Input("beta2_power")
            ...
            .Format(dataFormat)
            .UnknownShapeFormat(dataFormat);
        this->Input("lr").ParamType(REQUIRED).DataType(dataType).Format(dataFormat).UnknownShapeFormat(dataFormat);
        this->Input("weight_decay")
            .ParamType(REQUIRED)
            .DataType(dataType)
            .Format(dataFormat)
            .UnknownShapeFormat(dataFormat);
        this->Input("beta1").ParamType(REQUIRED).DataType(dataType).Format(dataFormat).UnknownShapeFormat(dataFormat);
        this->Input("beta2").ParamType(REQUIRED).DataType(dataType).Format(dataFormat).UnknownShapeFormat(dataFormat);
        this->Input("epsilon").ParamType(REQUIRED).DataType(dataType).Format(dataFormat).UnknownShapeFormat(dataFormat);
