...
public:
    ...
    __aicore__ inline void WeightReduceSumTailN(uint64_t targetNum, uint64_t peerLoopNIdx,
                                                LocalTensor<float>& weightYn);
...
__aicore__ inline void CrossEntropyLossGradBase<T1, T2>::InitUB()
{
    ...
    inOutQueInputSize = this->alignNCNum > this->colLoopNumTail ? this->alignNCNum :
...
template <typename T1, typename T2>
__aicore__ inline void CrossEntropyLossGradBase<T1, T2>::CopyInLog(uint64_t nLoopIdx, uint64_t cLoopIdx,
                                                                   uint64_t calcLen)
{
    ...
    DataCopyPad(logProbLocal, this->logProbGm[nLoopIdx * this->colVal + cLoopIdx * this->alignColLoopNum], copyParams,
                padParams);
    inQueLogProb_.EnQue(logProbLocal);
}

template <typename T1, typename T2>
__aicore__ inline void CrossEntropyLossGradBase<T1, T2>::CopyOutLog(uint64_t nLoopIdx, uint64_t cLoopIdx,
                                                                    uint64_t calcLen)
{
...
{
    DataCopyPad(this->workspaceGm[this->coreIndex], this->blockBuf2Local,
                {1, static_cast<uint32_t>(1 * sizeof(float)), 0, 0, 0});
    SyncAll();
    DataCopyPad(this->weightMaskLocal, this->workspaceGm,
                {1, static_cast<uint32_t>(this->usedCoreNum * sizeof(float)), 0, 0, 0}, {false, 0, 0, 0});
    this->PipeM2V();
    uint32_t srcShapeWs[2] = {static_cast<uint32_t>(1), static_cast<uint32_t>(this->usedCoreNum)};
    ReduceSum<float, AscendC::Pattern::Reduce::AR, true>(this->blockBuf2Local, this->weightMaskLocal, srcShapeWs,
                                                         false);
}
...
__aicore__ inline void CrossEntropyLossGradBase<T1, T2>::TargetWeightReduceSum(LocalTensor<float>& targetWeightLocal)
{
    ...
    for (uint64_t n = 0; n < this->nLoopTimes; n++) {
        ...
        if (this->ignoreIndex >= 0) {
            ...
            ReduceSum<float, AscendC::Pattern::Reduce::AR, true>(this->blockBuf3Local, this->weightMaskLocal, srcShape,
                                                                 false);
        } else {
...
template <typename T1, typename T2>
__aicore__ inline void CrossEntropyLossGradBase<T1, T2>::TargetWeight(uint64_t targetOffset, uint64_t targetNum,
                                                                      LocalTensor<float>& targetWeightLocal)
{
...
template <typename T1, typename T2>
__aicore__ inline void CrossEntropyLossGradBase<T1, T2>::WeightReduceSumTailN(uint64_t targetNum, uint64_t peerLoopNIdx,
                                                                              LocalTensor<float>& weightYn)
{
    if (peerLoopNIdx == 0) {
    ...
    SyncAll();
#endif
    DataCopyPad(this->weightMaskLocal, workspaceGm, {1, (uint32_t)(this->usedCoreNum * sizeof(float)), 0, 0, 0},
                {false, 0, 0, 0});
    this->PipeM2V();
    uint32_t srcShapeWs[2] = {static_cast<uint32_t>(1), static_cast<uint32_t>(this->usedCoreNum)};
    ReduceSum<float, AscendC::Pattern::Reduce::AR, true>(this->blockBuf2Local, this->weightMaskLocal, srcShapeWs,
...
template <typename T1, typename T2>
__aicore__ inline void CrossEntropyLossGradBase<T1, T2>::GradReductionNoneSmooth(uint64_t targetNum,
