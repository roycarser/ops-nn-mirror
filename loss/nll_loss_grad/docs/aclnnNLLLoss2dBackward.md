...
## 函数原型
  ...
  每个算子分为[两段式接口](../../../docs/zh/context/two_phase_api.md)，必须先调用“aclnnNLLLoss2dBackwardGetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnNLLLoss2dBackward”接口执行计算。
...
int main() {
  ...
  // 5. 获取输出的值，将device侧内存上的结果拷贝至host侧内存，需要根据具体API的接口定义修改
