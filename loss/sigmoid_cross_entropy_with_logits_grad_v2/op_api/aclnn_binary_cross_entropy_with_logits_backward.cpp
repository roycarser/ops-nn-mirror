...
static bool CheckNotNull(const aclTensor* gradOutput, const aclTensor* self, const aclTensor* target, aclTensor* out)
{
                               ...
                               aclTensor* out)
{
    ...
    CHECK_RET(CheckDtypeValid(gradOutput, self, target, weightOptional, posWeightOptional, out),
              ACLNN_ERR_PARAM_INVALID);
...
static aclnnStatus BinaryCrossEntropyWithLogitsBackwardStub(const aclTensor* gradOutput, const aclTensor* self,
...
aclnnStatus aclnnBinaryCrossEntropyWithLogitsBackwardGetWorkspaceSize(const aclTensor* gradOutput,
                                                                      ...
                                                                      uint64_t* workspaceSize, aclOpExecutor** executor)
{
    L2_DFX_PHASE_1(aclnnBinaryCrossEntropyWithLogitsBackward,
                   DFX_IN(gradOutput, self, target, weightOptional, posWeightOptional, reduction), DFX_OUT(out));
...
aclnnStatus aclnnBinaryCrossEntropyWithLogitsBackward(void* workspace, uint64_t workspaceSize, aclOpExecutor* executor,
