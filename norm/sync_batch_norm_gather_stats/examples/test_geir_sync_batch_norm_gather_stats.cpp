...
/*!
 * \file test_geir_sync_batch_norm_gather_stats.cpp
 * \brief
 */
...
#define ADD_INPUT(inputIndex, inputName, inputDtype, inputShape)                                             \
    ...
    SyncBatchNormGatherStats_1.set_input_##inputName(placeholder##inputIndex);                                            \
    inputs.push_back(placeholder##inputIndex);

#define ADD_INPUT_ATTR(attrName, attrValue) SyncBatchNormGatherStats_1.set_attr_##attrName(attrValue);
...
#define ADD_CONST_INPUT(inputIndex, inputName, inputDtype, inputShape)                                               \
    ...
    graph.AddOp(placeholder##inputIndex);                                                                            \
    SyncBatchNormGatherStats_1.set_input_##inputName(placeholder##inputIndex);                                                    \
    SyncBatchNormGatherStats_1.update_input_desc_##inputName(placeholder##inputIndex##_desc);                                     \
    inputs.push_back(placeholder##inputIndex);

#define ADD_OUTPUT(outputIndex, outputName, outputDtype, outputShape)                                        \
    TensorDesc outputName##outputIndex##_desc_ = TensorDesc(ge::Shape(outputShape), FORMAT_ND, outputDtype); \
    SyncBatchNormGatherStats_1.update_output_desc_##outputName(outputName##outputIndex##_desc_);
    ...
    Graph& graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto SyncBatchNormGatherStats_1 = op::SyncBatchNormGatherStats("SyncBatchNormGatherStats_1");
    std::vector<int64_t> totalSumShape = {2, 128};
    std::vector<int64_t> totalSquareSumShape = {2, 128};
    std::vector<int64_t> sampleCountShape = {2};
    std::vector<int64_t> meanShape = {128};
    std::vector<int64_t> varianceShape = {128};

    std::vector<int64_t> batchMeanShape = {128};
    std::vector<int64_t> batchInvstdShape = {128};
    std::vector<int64_t> meanShape = {128};
    std::vector<int64_t> varianceShape = {128};
    ADD_INPUT(1, total_sum, inDtype, totalSumShape);
    ADD_INPUT(2, total_square_sum, inDtype, totalSquareSumShape);
    ADD_INPUT(3, sample_count, DT_INT32, sampleCountShape);
    ADD_INPUT(4, mean, inDtype, meanShape);
    ADD_INPUT(5, variance, inDtype, varianceShape);
    ADD_OUTPUT(1, batch_mean, inDtype, batchMeanShape);
    ADD_OUTPUT(2, batch_invstd, batch_invstd, batchInvstdShape);
    ADD_OUTPUT(3, mean, batch_invstd, meanShape);
    ADD_OUTPUT(4, variance, batch_invstd, varianceShape);
    ADD_INPUT_ATTR(momentum, 0.001);
    ADD_INPUT_ATTR(eps, 0.001);
    outputs.push_back(SyncBatchNormGatherStats_1);
    // 添加完毕
    return SUCCESS;
}
