...
/*!
 * \file test_geir_sync_batch_norm_backward_elemt.cpp
 * \brief
 */
...
#include "../op_graph/sync_batch_norm_backward_elemt_proto.h"
...
#define ADD_INPUT(inputIndex, inputName, inputDtype, inputShape)                                             \
    ...
    SyncBatchNormBackwardElemt_1.set_input_##inputName(placeholder##inputIndex);                                            \
    inputs.push_back(placeholder##inputIndex);

#define ADD_INPUT_ATTR(attrName, attrValue) SyncBatchNormBackwardElemt_1.set_attr_##attrName(attrValue);
...
#define ADD_CONST_INPUT(inputIndex, inputName, inputDtype, inputShape)                                               \
    ...
    graph.AddOp(placeholder##inputIndex);                                                                            \
    SyncBatchNormBackwardElemt_1.set_input_##inputName(placeholder##inputIndex);                                                    \
    SyncBatchNormBackwardElemt_1.update_input_desc_##inputName(placeholder##inputIndex##_desc);                                     \
    inputs.push_back(placeholder##inputIndex);

#define ADD_OUTPUT(outputIndex, outputName, outputDtype, outputShape)                                        \
    TensorDesc outputName##outputIndex##_desc_ = TensorDesc(ge::Shape(outputShape), FORMAT_ND, outputDtype); \
    SyncBatchNormBackwardElemt_1.update_output_desc_##outputName(outputName##outputIndex##_desc_);
    ...
    Graph& graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto SyncBatchNormBackwardElemt_1 = op::SyncBatchNormBackwardElemt("SyncBatchNormBackwardElemt_1");
    std::vector<int64_t> gradOutputShape = {2, 128};
    std::vector<int64_t> saveInputShape = {2, 128};
    std::vector<int64_t> meanShape = {2, 128};
    std::vector<int64_t> invstdShape = {2, 128};
    std::vector<int64_t> weightShape = {2, 128};
    std::vector<int64_t> meanDyShape = {2, 128};
    std::vector<int64_t> meanDyXmuShape = {2, 128};
    std::vector<int64_t> gradInputShape = {2, 128};
    ADD_INPUT(1, grad_output, inDtype, gradOutputShape);
    ADD_INPUT(2, save_input, inDtype, saveInputShape);
    ADD_INPUT(3, mean, inDtype, meanShape);
    ADD_INPUT(4, invstd, inDtype, invstdShape);
    ADD_INPUT(5, weight, inDtype, weightShape);
    ADD_INPUT(6, mean_dy, inDtype, meanDyShape);
    ADD_INPUT(7, mean_dy_xmu, inDtype, meanDyXmuShape);
    ADD_OUTPUT(1, grad_input, inDtype, gradInputShape);
    outputs.push_back(SyncBatchNormBackwardElemt_1);
    // 添加完毕
    return SUCCESS;
}
