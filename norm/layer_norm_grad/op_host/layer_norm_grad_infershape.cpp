...
/*!
 * \file layer_norm_grad_infershape.cpp
 * \brief
 */
...
static const int64_t INPUT_VARIANCE_INDEX = 2;
...
static ge::graphStatus InferShapeLayerNormGrad(gert::InferShapeContext* context)
{
    if (context == nullptr) {
        OP_LOGE("LayerNormGrad", "InferShapeContext is nullptr");
        return ge::GRAPH_FAILED;
    }
    OP_LOGD(context, "Begin to do InferShapeLayerNormGrad.");
    ...
    const gert::Shape* variance_shape = context->GetInputShape(INPUT_VARIANCE_INDEX);
    ...
    OP_LOGD(context, "End to do InferShapeLayerNormGrad.");
    return ge::GRAPH_SUCCESS;
}
static ge::graphStatus InferDataTypeLayerNormGrad(gert::InferDataTypeContext* context)
{
    OP_LOGD(context, "Begin to do InferDataTypeLayerNormGrad");
...
IMPL_OP_INFERSHAPE(LayerNormGrad).InferShape(InferShapeLayerNormGrad).InferDataType(InferDataTypeLayerNormGrad);
} // namespace ops
