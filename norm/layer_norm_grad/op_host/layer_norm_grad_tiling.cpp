...
/*!
 * \file layer_norm_grad_tiling.cc
 * \brief
 */
#include<iostream>
#include<string>
...
static ge::graphStatus Tiling4LayerNormGrad(gert::TilingContext *context)
{
    return Ops::NN::Optiling::TilingRegistry::GetInstance().DoTilingImpl(context);
}

static ge::graphStatus TilingPrepare4LayerNormGrad(gert::TilingParseContext *context)
{
    OP_LOGD(context, "TilingPrepare4LayerNormGrad enter.");

    auto compileInfo = context->GetCompiledInfo<LayerNormGradCompileInfo>();
    ...
    OP_LOGD(context->GetNodeName(),
            "TilingPrepare4LayerNormGrad exit, coreNum: %u, blockSize: %ld, ubSize: %ld, vlFp32: %ld.",
            compileInfo->coreNum, compileInfo->blockSize, compileInfo->ubSizePlatForm, compileInfo->vlFp32);
    return ge::GRAPH_SUCCESS;
}

IMPL_OP_OPTILING(LayerNormGrad)
    .Tiling(Tiling4LayerNormGrad)
    .TilingParse<LayerNormGradCompileInfo>(TilingPrepare4LayerNormGrad);
} // namespace optiling
