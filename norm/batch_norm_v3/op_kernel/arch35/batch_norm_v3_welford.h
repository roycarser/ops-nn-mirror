...
public:
                                ...
                                GM_ADDR mean_out, GM_ADDR var_out, GM_ADDR batch_mean, GM_ADDR batch_rstd)
    {
        ...
        momentumForVar = elemNum == 1 ? AscendC::NumericLimits<float>::QuietNaN() : momentum * elemNum / (elemNum - 1);
        ...
        int64_t aGatherLimitBetaGamaAlign = RoundUp32(aGatherLimit, sizeof(T_BETA));
        pipe.InitBuffer(betaQueue, BUFFER_NUM, aGatherLimitBetaGamaAlign * sizeof(T_BETA));
        pipe.InitBuffer(gammaQueue, BUFFER_NUM, aGatherLimitBetaGamaAlign * sizeof(T_BETA));
    ...
    __aicore__ inline void ProcessCutR0(__ubuf__ float* tmpMeanLocal, __ubuf__ float* tmpVarLocal,
                                        __ubuf__ float* meanLocal, __ubuf__ float* rstdLocal, __ubuf__ float* varLocal,
                                        __ubuf__ float* dichotomyAddLocal, int32_t aParallelIndex,
                                        int32_t aParallelLength)
    {
    ...
    __aicore__ inline void ProcessCutR1(__ubuf__ float* tmpMeanLocal, __ubuf__ float* tmpVarLocal,
                                        __ubuf__ float* meanLocal, __ubuf__ float* rstdLocal, __ubuf__ float* varLocal,
                                        __ubuf__ float* dichotomyAddLocal, int32_t aParallelIndex,
                                        int32_t aParallelLength)
    {
    ...
    __aicore__ inline void ProcessAParallel(int32_t aParallelIndex, int32_t aParallelLength)
    {
        ...
        __ubuf__ float* batchMeanLocal = (__ubuf__ float*)batchMeanTensor.GetPhyAddr();
        __ubuf__ float* batchVarLocal = (__ubuf__ float*)batchVarTensor.GetPhyAddr();
        __ubuf__ float* batchRstdLocal = (__ubuf__ float*)batchRstdTensor.GetPhyAddr();
        ...
        __ubuf__ T_RUNNING_MEAN* meanLocal = (__ubuf__ T_RUNNING_MEAN*)meanTensor.GetPhyAddr();
        __ubuf__ T_RUNNING_MEAN* varLocal = (__ubuf__ T_RUNNING_MEAN*)varTensor.GetPhyAddr();
        __ubuf__ T_RUNNING_MEAN* outMeanLocal = (__ubuf__ T_RUNNING_MEAN*)outMeanTensor.GetPhyAddr();
        __ubuf__ T_RUNNING_MEAN* outVarLocal = (__ubuf__ T_RUNNING_MEAN*)outVarTensor.GetPhyAddr();

        uint16_t loopCount2 = CeilDiv(aParallelLength, VL_FP32);

        __ubuf__ float* batchMeanLocal2 = (__ubuf__ float*)batchMeanTensor.GetPhyAddr();
        __ubuf__ float* batchVarLocal2 = (__ubuf__ float*)batchVarTensor.GetPhyAddr();
...
private:
    __aicore__ inline void ProcessSplitWelfordUpdate(__ubuf__ float* tmpMeanLocal, __ubuf__ float* tmpVarLocal,
                                                     int64_t index, int64_t nBurst, int64_t burstLen, int64_t count,
                                                     bool isFirstStep)
    {
    ...
    __aicore__ inline void CopySmallTensor2UB(const int64_t& index, const int64_t& copyLen)
    {
        ...
        DataCopyPadExtParams<T_BETA> padParamsBetaGamma;
        padParamsBetaGamma.isPad = true;
        padParamsBetaGamma.paddingValue = static_cast<T_BETA>(0.0);
        ...
        dataCopyParamsBetaGamma.blockLen = copyLen * sizeof(T_BETA);
    ...
    __aicore__ inline void CalculateRunningMeanAndVar(__ubuf__ float* batchMeanLocal, __ubuf__ float* batchVarLocal,
                                                      __ubuf__ T_RUNNING_MEAN* meanLocal,
                                                      __ubuf__ T_RUNNING_MEAN* varLocal,
                                                      __ubuf__ T_RUNNING_MEAN* outMeanLocal,
                                                      __ubuf__ T_RUNNING_MEAN* outVarLocal, uint16_t loopCount,
                                                      uint32_t calLen)
    {
        __VEC_SCOPE__
        {
            ...
            for (uint16_t i = 0; i < loopCount; i++) {
                ...
                if constexpr (!IsSameType<T_RUNNING_MEAN, float>::value) {
                    ...
                    AscendC::MicroAPI::LoadAlign<T_RUNNING_MEAN, AscendC::MicroAPI::LoadDist::DIST_UNPACK_B16>(
                        meanTmp, ((__ubuf__ T_RUNNING_MEAN*)meanLocal + i * VL_FP32));
                    AscendC::MicroAPI::Cast<float, T_RUNNING_MEAN, NormCommon::castTraitB162B32>(mean, meanTmp,
                                                                                                 pregLoop);
                } else {
                    AscendC::MicroAPI::LoadAlign(mean, ((__ubuf__ float*)meanLocal + i * VL_FP32));
                }
                ...
                if constexpr (!IsSameType<T_RUNNING_MEAN, float>::value) {
                    ...
                    AscendC::MicroAPI::Cast<T_RUNNING_MEAN, float, NormCommon::castTraitB322B16>(outMeanTmp, mean,
                                                                                                 pregLoop);
                    AscendC::MicroAPI::StoreAlign<T_RUNNING_MEAN, AscendC::MicroAPI::StoreDist::DIST_PACK_B32>(
                        ((__ubuf__ T_RUNNING_MEAN*)outMeanLocal + i * VL_FP32), outMeanTmp, pregLoop);
                } else {
                    AscendC::MicroAPI::StoreAlign(((__ubuf__ float*)outMeanLocal + i * VL_FP32), mean, pregLoop);
                }
                ...
                if constexpr (!IsSameType<T_RUNNING_MEAN, float>::value) {
                    ...
                    AscendC::MicroAPI::LoadAlign<T_RUNNING_MEAN, AscendC::MicroAPI::LoadDist::DIST_UNPACK_B16>(
                        varTmp, ((__ubuf__ T_RUNNING_MEAN*)varLocal + i * VL_FP32));
                    AscendC::MicroAPI::Cast<float, T_RUNNING_MEAN, NormCommon::castTraitB162B32>(var, varTmp, pregLoop);
                } else {
                    AscendC::MicroAPI::LoadAlign(var, ((__ubuf__ float*)varLocal + i * VL_FP32));
                }
                LoadAlign(batchVar, batchVarLocal + i * VL_FP32);
                ...
                if constexpr (!IsSameType<T_RUNNING_MEAN, float>::value) {
                    ...
                    AscendC::MicroAPI::Cast<T_RUNNING_MEAN, float, NormCommon::castTraitB322B16>(outVarTmp, var,
                                                                                                 pregLoop);
                    AscendC::MicroAPI::StoreAlign<T_RUNNING_MEAN, AscendC::MicroAPI::StoreDist::DIST_PACK_B32>(
                        ((__ubuf__ T_RUNNING_MEAN*)outVarLocal + i * VL_FP32), outVarTmp, pregLoop);
                } else {
                    AscendC::MicroAPI::StoreAlign(((__ubuf__ float*)outVarLocal + i * VL_FP32), var, pregLoop);
                }
            }
        }
    }
    ...
    */
    __aicore__ inline void VFWelfordParallelFinalizeAlign(
        __ubuf__ float* meanLocal, __ubuf__ float* rstdLocal, __ubuf__ float* varLocal, __ubuf__ float* tmpMeanLocal,
        __ubuf__ float* tmpVarLocal, __ubuf__ float* dichotomyAddLocal, uint32_t reduceCount,
        uint32_t dichotomyAddPower, uint32_t dichotomyAddK, uint32_t dichotomyAddLastNum, uint32_t offset,
        float reduceScale0, float reduceScale1, float scale0, float scale1, float cnt, float eps)
    {
        ...
        __VEC_SCOPE__
        {
            ...
            StoreAlign<float, StoreDist::DIST_FIRST_ELEMENT_B32>(varLocal + offset, var, pregMerge);
            NormCommon::ComputeRstdNewtonRaphsonReg<false>(var, rstd, pregMerge, eps);
            StoreAlign<float, StoreDist::DIST_FIRST_ELEMENT_B32>(rstdLocal + offset, rstd, pregMerge);
        }
    }
    /*
    ...
    */
    ...
    __aicore__ inline void VFWelfordParallelFinalizeNonAlignSituation1(
        ...
        __ubuf__ float* tmpVarLocal, __ubuf__ float* dichotomyAddLocal, uint32_t reduceCount,
        uint32_t dichotomyAddPower, uint32_t dichotomyAddK, uint32_t dichotomyAddLastNum, uint32_t offset,
        uint32_t tailSize, float reduceScale0, float reduceScale1, float cnt, float tailCnt, float eps)
    {
        // cnt  是不带尾块的累加次数
        ...
        __VEC_SCOPE__
        {
            ...
            for (uint16_t i = 0; i < welfordReminderLoopCount; i++) {
                ...
                Move<float, MaskMergeMode::MERGING>(dichotomyAddMeanR, tmp, pregLoop1);
                Add(sumMean, dichotomyAddMeanL, dichotomyAddMeanR, pregMain);
                Reduce<ReduceType::SUM>(mean, sumMean, pregMain);
            ...
            for (uint16_t i = 0; i < welfordReminderLoopCount; i++) {
                ...
                Move<float, MaskMergeMode::MERGING>(deltaR, tmp, pregLoop1);
            ...
            for (uint16_t i = 0; i < dichotomyAddReminderLoopCount; i++) {
                ...
                Add(dichotomyAddVarL, dichotomyAddVarL, deltaL, pregMain);
                Muls(dichotomyAddVarL, dichotomyAddVarL, reduceScale0, pregMain);
                LoadAlign(dichotomyAddVarR, tmpVarLocal + (i + welfordDiffLoopCount) * VL_FP32 +
                                                welfordDiffReminderAlign + dichotomyAddPower);
                Add(dichotomyAddVarR, dichotomyAddVarR, deltaR, pregLoop);
                Muls(dichotomyAddVarR, dichotomyAddVarR, reduceScale0, pregLoop);
                Add(sumVar, dichotomyAddVarL, dichotomyAddVarR, pregMain);
        ...
        uint32_t tailSize, float reduceScale0, float reduceScale1, float cnt, float tailCnt, float eps)
    {
        ...
        __VEC_SCOPE__
        {
            ...
            for (uint16_t i = 0; i < welfordReminderLoopCount; i++) {
                ...
                Move<float, MaskMergeMode::MERGING>(dichotomyAddMeanL, tmp, pregLoop1);
                Add(sumMean, dichotomyAddMeanL, dichotomyAddMeanR, pregMain);
                Reduce<ReduceType::SUM>(mean, sumMean, pregMain);
                StoreAlign<float, StoreDist::DIST_FIRST_ELEMENT_B32>(dichotomyAddLocal + i + welfordDiffLoopCount, mean,
                                                                     pregMerge);
            ...
            for (uint16_t i = 0; i < welfordReminderLoopCount; i++) {
                ...
                Move<float, MaskMergeMode::MERGING>(deltaL, tmp, pregLoop1);
            ...
            for (uint16_t i = 0; i < dichotomyAddReminderLoopCount; i++) {
                MaskReg pregMask = AscendC::MicroAPI::UpdateMask<float>(sreg0);
                LoadAlign(dichotomyAddMeanL,
                          tmpMeanLocal + (i + welfordDiffLoopCount) * VL_FP32 + welfordDiffReminderAlign);
                Sub(deltaL, dichotomyAddMeanL, mean, pregMain);
                Mul(deltaL, deltaL, deltaL, pregMain);
                ...
                LoadAlign(dichotomyAddMeanR, tmpMeanLocal + (i + welfordDiffLoopCount) * VL_FP32 +
                                                 welfordDiffReminderAlign + dichotomyAddPower);
                Sub(deltaR, dichotomyAddMeanR, mean, pregMask);
                ...
                Muls(dichotomyAddVarR, dichotomyAddVarR, reduceScale0, pregMask);
                Add(sumVar, dichotomyAddVarL, dichotomyAddVarR, pregMain);
                Reduce<ReduceType::SUM>(var, sumVar, pregMain);
                StoreAlign<float, StoreDist::DIST_FIRST_ELEMENT_B32>(
                    dichotomyAddLocal + i + welfordDiffLoopCount + welfordReminderLoopCount, var, pregMerge);
        ...
        uint32_t tailSize, float reduceScale0, float reduceScale1, float cnt, float tailCnt, float eps)
    {
        ...
        __VEC_SCOPE__
        {
            RegTensor<float> dichotomyAddMeanL;
            RegTensor<float> dichotomyAddMeanR;
            RegTensor<float> dichotomyAddVarLReg;
            RegTensor<float> dichotomyAddVarRReg;
            RegTensor<float> sumMean;
            RegTensor<float> mean;
            RegTensor<float> sumVarReg;
            RegTensor<float> varReg;
            RegTensor<float> deltaLReg;
            RegTensor<float> deltaRReg;
            RegTensor<float> one;
            ...
            for (uint16_t i = 0; i < welfordReminderLoopCount; i++) {
                ...
                Move<float, MaskMergeMode::MERGING>(dichotomyAddMeanL, tmp, pregLoop);
                Reduce<ReduceType::SUM>(mean, dichotomyAddMeanL, pregMain);
                StoreAlign<float, StoreDist::DIST_FIRST_ELEMENT_B32>(
            ...
            for (uint16_t i = 0; i < dichotomyAddReminderLoopCount; i++) {
                pregLoop = AscendC::MicroAPI::UpdateMask<float>(sreg0);
                LoadAlign(dichotomyAddMeanL, tmpMeanLocal + i * VL_FP32);
                Sub(deltaLReg, dichotomyAddMeanL, mean, pregMain);
                Mul(deltaLReg, deltaLReg, deltaLReg, pregMain);
                Muls(deltaLReg, deltaLReg, tailCnt, pregMain);
                LoadAlign(dichotomyAddVarLReg, tmpVarLocal + i * VL_FP32);
                Add(dichotomyAddVarLReg, dichotomyAddVarLReg, deltaLReg, pregMain);
                Muls(dichotomyAddVarLReg, dichotomyAddVarLReg, reduceScale0, pregMain);

                LoadAlign(dichotomyAddMeanR, tmpMeanLocal + i * VL_FP32 + dichotomyAddPower);
                Sub(deltaRReg, dichotomyAddMeanR, mean, pregLoop);
                Mul(deltaRReg, deltaRReg, deltaRReg, pregLoop);
                Muls(deltaRReg, deltaRReg, cnt, pregLoop);
                LoadAlign(dichotomyAddVarRReg, tmpVarLocal + i * VL_FP32 + dichotomyAddPower);
                Add(dichotomyAddVarRReg, dichotomyAddVarRReg, deltaRReg, pregLoop);
                Muls(dichotomyAddVarRReg, dichotomyAddVarRReg, reduceScale0, pregLoop);

                Add(sumVarReg, dichotomyAddVarLReg, dichotomyAddVarRReg, pregMain);
                Reduce<ReduceType::SUM>(varReg, sumVarReg, pregMain);
                StoreAlign<float, StoreDist::DIST_FIRST_ELEMENT_B32>(dichotomyAddLocal + i, varReg, pregMerge);
            }
            ...
            for (uint16_t i = 0; i < welfordDiffLoopCount; i++) {
                ...
                Mul(deltaLReg, deltaLReg, deltaLReg, pregMain);
                Muls(deltaLReg, deltaLReg, tailCnt, pregMain);
                LoadAlign(dichotomyAddVarLReg, tmpVarLocal + i * VL_FP32 + dichotomyAddReminderRoundUp);
                Add(dichotomyAddVarLReg, dichotomyAddVarLReg, deltaLReg, pregMain);
                Muls(dichotomyAddVarLReg, dichotomyAddVarLReg, reduceScale0, pregMain);
                Reduce<ReduceType::SUM>(varReg, dichotomyAddVarLReg, pregMain);
                StoreAlign<float, StoreDist::DIST_FIRST_ELEMENT_B32>(
            ...
            for (uint16_t i = 0; i < welfordReminderLoopCount; i++) {
                ...
                Sub(deltaLReg, dichotomyAddMeanL, mean, pregMain);
                Mul(deltaLReg, deltaLReg, deltaLReg, pregMain);
                Muls(deltaLReg, deltaLReg, cnt, pregMain);
                Muls(tmp, deltaLReg, coeff, pregLoop);
                Move<float, MaskMergeMode::MERGING>(deltaLReg, tmp, pregLoop);
                LoadAlign(dichotomyAddVarLReg,
            ...
            for (uint16_t i = 0; i < dichotomyAddPowerRemainLoopCount; i++) {
                ...
                Mul(deltaLReg, deltaLReg, deltaLReg, pregMain);
                Muls(deltaLReg, deltaLReg, cnt, pregMain);
                LoadAlign(dichotomyAddVarLReg, tmpVarLocal + i * VL_FP32 + dichotomyAddPowerOffset);
                Add(dichotomyAddVarLReg, dichotomyAddVarLReg, deltaLReg, pregMain);
                Muls(dichotomyAddVarLReg, dichotomyAddVarLReg, reduceScale0, pregMain);
                Reduce<ReduceType::SUM>(varReg, dichotomyAddVarLReg, pregMain);
                StoreAlign<float, StoreDist::DIST_FIRST_ELEMENT_B32>(dichotomyAddLocal + dichotomyAddReminderLoopCount +
            ...
            NormCommon::DichotomyAdd(varReg, dichotomyAddLocal, dichotomyAddK, innerLoopCountOrigin,
                                     dichotomyAddLastNum);
            Muls(varReg, varReg, reduceScale1, pregMerge);
            StoreAlign<float, StoreDist::DIST_FIRST_ELEMENT_B32>(varLocal + offset, varReg, pregMerge);
            NormCommon::ComputeRstdNewtonRaphsonReg<false>(varReg, rstdReg, pregMerge, eps);
            StoreAlign<float, StoreDist::DIST_FIRST_ELEMENT_B32>(rstdLocal + offset, rstdReg, pregMerge);
        }
    }
    ...
    {
        LocalTensor<T_BETA> betaTensor = betaQueue.DeQue<T_BETA>();
        LocalTensor<T_BETA> gammaTensor = gammaQueue.DeQue<T_BETA>();
        LocalTensor<float> batchMeanTensor = batchMeanQueue.DeQue<float>();
        LocalTensor<float> batchRstdTensor = batchRstdQueue.DeQue<float>();
        __ubuf__ T_BETA* gammaLocal = (__ubuf__ T_BETA*)gammaTensor.GetPhyAddr();
        __ubuf__ T_BETA* betaLocal = (__ubuf__ T_BETA*)betaTensor.GetPhyAddr();
        __ubuf__ float* meanLocal = (__ubuf__ float*)batchMeanTensor.GetPhyAddr();
        __ubuf__ float* rstdLocal = (__ubuf__ float*)batchRstdTensor.GetPhyAddr();
        ...
        for (int64_t i = 0; i < aParallelLength; i++) {
            for (uint64_t j = 0; j < loopR1outer; j++) {
                ...
                for (uint64_t k = 0; k < loopR0outer; k++) {
                    ...
                    __ubuf__ T* xLocal = (__ubuf__ T*)xInUb.GetPhyAddr();
                    __ubuf__ T* yOutLocal = (__ubuf__ T*)yOutUb.GetPhyAddr();
    ...
    __aicore__ inline void VFNormalize(__ubuf__ T* xLocal, __ubuf__ T_BETA* gammaLocal, __ubuf__ T_BETA* betaLocal,
                                       __ubuf__ float* meanLocal, __ubuf__ float* rstdLocal, __ubuf__ T* yLocal,
                                       int64_t i, uint16_t loopCount, int32_t reduceCount)
    {
        __VEC_SCOPE__
        {
            RegTensor<float> x;
            ...
            for (uint16_t j = 0; j < loopCount; j++) {
                ...
                if constexpr (IsSameType<T, half>::value) {
                    ...
                    Cast<half, float, NormCommon::castTraitB322B16>(yFp16, y, pregLoop);
                    StoreAlign<half, StoreDist::DIST_PACK_B32>(((__ubuf__ half*)yLocal) + j * VL_FP32, yFp16, pregLoop);
                } else if constexpr (IsSameType<T, bfloat16_t>::value) {
                    RegTensor<bfloat16_t> xBf16;
                    Cast<bfloat16_t, float, NormCommon::castTraitB322B16>(xBf16, y, pregLoop);
                    StoreAlign<bfloat16_t, StoreDist::DIST_PACK_B32>(((__ubuf__ bfloat16_t*)yLocal) + j * VL_FP32,
                                                                     xBf16, pregLoop);
                } else {
                    StoreAlign(((__ubuf__ float*)yLocal) + j * VL_FP32, y, pregLoop);
                }
            }
        }
    }
