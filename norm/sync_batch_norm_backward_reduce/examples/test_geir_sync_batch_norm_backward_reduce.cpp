...
/*!
 * \file test_geir_sync_batch_norm_backward_reduce.cpp
 * \brief
 */
...
#define ADD_INPUT(inputIndex, inputName, inputDtype, inputShape)                                             \
    ...
    SyncBatchNormBackwardReduce_1.set_input_##inputName(placeholder##inputIndex);                                            \
    inputs.push_back(placeholder##inputIndex);

#define ADD_INPUT_ATTR(attrName, attrValue) SyncBatchNormBackwardReduce_1.set_attr_##attrName(attrValue);
...
#define ADD_CONST_INPUT(inputIndex, inputName, inputDtype, inputShape)                                               \
    ...
    graph.AddOp(placeholder##inputIndex);                                                                            \
    SyncBatchNormBackwardReduce_1.set_input_##inputName(placeholder##inputIndex);                                                    \
    SyncBatchNormBackwardReduce_1.update_input_desc_##inputName(placeholder##inputIndex##_desc);                                     \
    inputs.push_back(placeholder##inputIndex);

#define ADD_OUTPUT(outputIndex, outputName, outputDtype, outputShape)                                        \
    TensorDesc outputName##outputIndex##_desc_ = TensorDesc(ge::Shape(outputShape), FORMAT_ND, outputDtype); \
    SyncBatchNormBackwardReduce_1.update_output_desc_##outputName(outputName##outputIndex##_desc_);
    ...
    Graph& graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto SyncBatchNormBackwardReduce_1 = op::SyncBatchNormBackwardReduce("SyncBatchNormBackwardReduce_1");
    std::vector<int64_t> sumDyShape = {2, 128};
    std::vector<int64_t> sumDyDxPadShape = {2, 128};
    std::vector<int64_t> meanShape = {2, 128};
    std::vector<int64_t> invertStdShape = {2, 128};
    std::vector<int64_t> sumDyXmuShape = {2, 128};
    std::vector<int64_t> yShape = {2, 128};
    ADD_INPUT(1, sum_dy, inDtype, sumDyShape);
    ADD_INPUT(2, sum_dy_dx_pad, inDtype, sumDyDxPadShape);
    ADD_INPUT(3, mean, inDtype, meanShape);
    ADD_INPUT(4, invert_std, inDtype, invertStdShape);
    ADD_OUTPUT(1, sum_dy_xmu, inDtype, sumDyXmuShape);
    ADD_OUTPUT(2, y, inDtype, yShape);
    outputs.push_back(SyncBatchNormBackwardReduce_1);
    // 添加完毕
    return SUCCESS;
}
