...
#define ADD_INPUT(inputIndex, inputName, inputDtype, inputShape)                                             \
    ...
    layerNorm_1.set_input_##inputName(placeholder##inputIndex);                                            \
    inputs.push_back(placeholder##inputIndex);

#define ADD_INPUT_ATTR(attrName, attrValue) layerNorm_1.set_attr_##attrName(attrValue);
...
#define ADD_CONST_INPUT(inputIndex, inputName, inputDtype, inputShape)                                               \
    ...
    graph.AddOp(placeholder##inputIndex);                                                                            \
    layerNorm_1.set_input_##inputName(placeholder##inputIndex);                                                    \
    layerNorm_1.update_input_desc_##inputName(placeholder##inputIndex##_desc);                                     \
    inputs.push_back(placeholder##inputIndex);

#define ADD_OUTPUT(outputIndex, outputName, outputDtype, outputShape)                                        \
    TensorDesc outputName##outputIndex##_desc_ = TensorDesc(ge::Shape(outputShape), FORMAT_ND, outputDtype); \
    layerNorm_1.update_output_desc_##outputName(outputName##outputIndex##_desc_);
    ...
    Graph& graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto layerNorm_1 = op::LayerNorm("layerNorm_1");
    std::vector<int64_t> xShape = {2, 128, 168};
    std::vector<int64_t> gammaShape = {168};
    std::vector<int64_t> betaShape = {168};
    ...
    ADD_OUTPUT(2, mean, inDtype, outShape);
    ADD_OUTPUT(3, variance, inDtype, outShape);
    ADD_INPUT_ATTR(begin_norm_axis, 2);
    ADD_INPUT_ATTR(begin_params_axis, 2);
    ADD_INPUT_ATTR(epsilon, 1e-5);
    outputs.push_back(layerNorm_1);
    // 添加完毕
    return SUCCESS;
}
