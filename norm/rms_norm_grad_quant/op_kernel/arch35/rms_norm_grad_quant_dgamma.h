...
/*!
 * \file rms_norm_grad_quant_regbase_dgamma.h
 * \brief RmsNormGradQuant regbase dgamma
 */

#ifndef RMS_NORM_GRAD_QUANT_DGAMMA_H
#define RMS_NORM_GRAD_QUANT_DGAMMA_H

#include "kernel_tiling/kernel_tiling.h"
#include "kernel_operator.h"
#include "rms_norm_grad_quant_dgamma_helper.h"

namespace RmsNormGradQuant {
using namespace AscendC;
template <typename DY_TYPE, typename X_TYPE, typename RSTD_TYPE, bool IS_FULL_LOAD, int32_t BUFFER_NUM = 2>
class RegbaseDgamma {
public:
    __aicore__ inline RegbaseDgamma(TPipe* pipe, const RmsNormGradQuantRegbaseTilingData* tilingData)
                                           ...
                                           uint16_t curUBLoopColsCount, int32_t curRowsNum)
    {
        ...
        __VEC_SCOPE__
        {
            ...
            for (uint16_t k = 0; k < static_cast<uint16_t>(curRowsNum); k++) {
                CalcMulRes<DY_TYPE, X_TYPE, RSTD_TYPE>(dyAddr, xAddr, rstdAddr, dgammaOutAddr, pregMain,
...
private:
    ...
    const RmsNormGradQuantRegbaseTilingData* tiling_;
};
} // namespace RmsNormGradQuant
