...
{
    uint32_t elemIndex1 = 0;
    for (; elemIndex1 + ELEM_PER_REP_FP32 <= numLastDim; elemIndex1 += ELEM_PER_REP_FP32) {
        Add(tmpLocal, srcLocal[elemIndex1], tmpLocal, elemNum, repeat,
            {1, 1, 1, ELEM_PER_BLK_FP32, repStride, ELEM_PER_BLK_FP32});
        PipeBarrier<PIPE_V>();
    }
    if (unlikely(tailCount != 0)) {
        Add(tmpLocal, srcLocal[elemIndex1], tmpLocal, tailCount, repeat,
            {1, 1, 1, ELEM_PER_BLK_FP32, repStride, ELEM_PER_BLK_FP32});
    }
    PipeBarrier<PIPE_V>();
...
__aicore__ inline void ReduceSumForSmallReduceDim(
    const LocalTensor<float>& dstLocal3, const LocalTensor<float>& srcLocal, const LocalTensor<float>& tmpLocal,
    ...
    const uint8_t repStride)
{
    ...
    if (repeatTimes == 0) {
        ReduceSumForSmallReduceDimPreRepeat(
            dstLocal3, srcLocal, tmpLocal, ELEM_PER_REP_FP32, numLastDim, tailCount, repeat, repStride);
    } else {
        ...
        for (; repIndex + MAX_REP_NUM <= repeat; repIndex += MAX_REP_NUM) {
            ReduceSumForSmallReduceDimPreRepeat(
                dstLocal3[repIndex], srcLocal[repIndex * numLastDimAligned], tmpLocal[repIndex * ELEM_PER_REP_FP32],
...
__aicore__ inline void ReduceSumMultiN(
    const LocalTensor<float>& dstLocal, const LocalTensor<float>& srcLocal, const LocalTensor<float>& tmpLocal3,
    const uint32_t numRow, const uint32_t numCol, const uint32_t numColAlign)
{
    ...
    Duplicate(tmpLocal3, ZERO, numRow * ELEM_PER_REP_FP32);
    PipeBarrier<PIPE_V>();
    ReduceSumForSmallReduceDim(dstLocal, srcLocal, tmpLocal3, numColAlign, numCol, tailCount, repeat, repStride);
}

__aicore__ inline int32_t findPowerTwo(int32_t n4)
{
    // find max power of 2 no more than n (32 bit)
    n4 |= n4 >> 1; // Set the first digit of n's binary to 1
    n4 |= n4 >> INDEX_TWO;
    n4 |= n4 >> INDEX_FOUR;
    n4 |= n4 >> INDEX_EIGHT;
    n4 |= n4 >> INDEX_SIXTEEN;
