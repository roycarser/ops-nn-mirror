...
/*!
 * \file test_geir_sync_bn_training_update.cpp
 * \brief
 */
...
#include "../op_graph/sync_bn_training_update_proto.h"
...
#define ADD_INPUT(inputIndex, inputName, inputDtype, inputShape)                                             \
    ...
    SyncBNTrainingUpdate_1.set_input_##inputName(placeholder##inputIndex);                                            \
    inputs.push_back(placeholder##inputIndex);

#define ADD_INPUT_ATTR(attrName, attrValue) SyncBNTrainingUpdate_1.set_attr_##attrName(attrValue);
...
#define ADD_CONST_INPUT(inputIndex, inputName, inputDtype, inputShape)                                               \
    ...
    graph.AddOp(placeholder##inputIndex);                                                                            \
    SyncBNTrainingUpdate_1.set_input_##inputName(placeholder##inputIndex);                                                    \
    SyncBNTrainingUpdate_1.update_input_desc_##inputName(placeholder##inputIndex##_desc);                                     \
    inputs.push_back(placeholder##inputIndex);

#define ADD_OUTPUT(outputIndex, outputName, outputDtype, outputShape)                                        \
    TensorDesc outputName##outputIndex##_desc_ = TensorDesc(ge::Shape(outputShape), FORMAT_ND, outputDtype); \
    SyncBNTrainingUpdate_1.update_output_desc_##outputName(outputName##outputIndex##_desc_);
    ...
    Graph& graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto SyncBNTrainingUpdate_1 = op::SyncBNTrainingUpdate("SyncBNTrainingUpdate_1");
    std::vector<int64_t> meanShape = {2, 128};
    std::vector<int64_t> runningMeanShape = {2, 128};
    std::vector<int64_t> runningMeanUpdateShape = {2, 128};
    ADD_INPUT(1, mean, inDtype, meanShape);
    ADD_INPUT(2, running_mean, inDtype, runningMeanShape);
    ADD_OUTPUT(1, running_mean_update, inDtype, runningMeanUpdateShape);
    ADD_INPUT_ATTR(momentum, 0.1);
    outputs.push_back(SyncBNTrainingUpdate_1);
    // 添加完毕
    return SUCCESS;
}
