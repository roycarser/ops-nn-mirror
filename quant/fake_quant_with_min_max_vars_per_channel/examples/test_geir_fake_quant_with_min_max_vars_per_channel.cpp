...
/*!
 * \file test_geir_fake_quant_with_min_max_vars_per_channel.cpp
 * \brief
 */
...
{
    Status ret = SUCCESS;
    auto fakeQuantOp = op::FakeQuantWithMinMaxVarsPerChannel("fakeQuantWithMinMaxVarsPerChannel1");

    std::vector<int64_t> xShape = {4, 4};
    std::vector<int64_t> channelShape = {4};
    ADD_INPUT(1, x, inDtype, xShape, true, -8.0f, 8.0f, 0.0f);
    ADD_INPUT(2, min, inDtype, channelShape, false, 0.0f, 0.0f, -6.0f);
    ADD_INPUT(3, max, inDtype, channelShape, false, 0.0f, 0.0f, 6.0f);
...
int main(int argc, char* argv[])
{
    ...
    for (int i = 0; i < output_num; i++) {
        ...
        LOG_PRINT("\n========== fake_quant_with_min_max_vars_per_channel result ==========\n");
        LOG_PRINT("x            y\n");
