...
/*!
 * \file test_geir_fake_quant_with_min_max_vars_gradient.cpp
 * \brief
 */
...
{
    Status ret = SUCCESS;
    auto fakeQuantGradOp = op::FakeQuantWithMinMaxVarsGradient("fakeQuantWithMinMaxVarsGradient1");

    std::vector<int64_t> shape = {16};
    std::vector<int64_t> minMaxShape = {1};
    ADD_INPUT(1, gradients, inDtype, shape, true, -1.0f, 1.0f, 0.0f);
    ADD_INPUT(2, x, inDtype, shape, true, -8.0f, 8.0f, 0.0f);
    ADD_INPUT(3, min, inDtype, minMaxShape, false, 0.0f, 0.0f, -6.0f);
    ADD_INPUT(4, max, inDtype, minMaxShape, false, 0.0f, 0.0f, 6.0f);

    ADD_INPUT_ATTR(num_bits, 8);
    ADD_INPUT_ATTR(narrow_range, false);

    ADD_OUTPUT(1, backprops_wrt_x, inDtype, shape);
    ADD_OUTPUT(2, backprop_wrt_min, inDtype, minMaxShape);
    ADD_OUTPUT(3, backprop_wrt_max, inDtype, minMaxShape);
...
int main(int argc, char* argv[])
{
    ...
    for (int i = 0; i < output_num; i++) {
        ...
        if (i == 0) {
            ...
            LOG_PRINT("\n========== fake_quant_with_min_max_vars_gradient result ==========\n");
            ...
            LOG_PRINT("backprop_wrt_%s = %f\n", (i == 1 ? "min" : "max"), resultData[0]);
        }
    }
