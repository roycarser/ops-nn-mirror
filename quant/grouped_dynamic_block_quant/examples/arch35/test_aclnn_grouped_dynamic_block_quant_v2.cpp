...
#include <vector>
#include "acl/acl.h"
#include "aclnnop/aclnn_grouped_dynamic_block_quant_v2.h"
...
int main()
{
    ...
    // 调用aclnnGroupedDynamicBlockQuantV2第一段接口
    ret = aclnnGroupedDynamicBlockQuantV2GetWorkspaceSize(
        x, groupList, minScale, (char*)roundMode, aclDataType::ACL_FLOAT8_E5M2, rowBlockSize, colBlockSize,
        groupListType, dstTypeMax, y, scale, &workspaceSize, &executor);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnGroupedDynamicBlockQuantV2GetWorkspaceSize failed. ERROR: %d\n", ret);
              return ret);
    ...
    // 调用aclnnGroupedDynamicBlockQuantV2第二段接口
    ret = aclnnGroupedDynamicBlockQuantV2(workspaceAddr, workspaceSize, executor, stream);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnGroupedDynamicBlockQuantV2 failed. ERROR: %d\n", ret); return ret);
