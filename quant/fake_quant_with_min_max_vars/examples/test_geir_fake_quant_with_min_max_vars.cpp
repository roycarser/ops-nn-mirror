...
/*!
 * \file test_geir_fake_quant_with_min_max_vars.cpp
 * \brief
 */
...
#include "../op_graph/fake_quant_with_min_max_vars_proto.h"
...
{
    Status ret = SUCCESS;
    auto fakeQuantOp = op::FakeQuantWithMinMaxVars("fakeQuantWithMinMaxVars1");

    std::vector<int64_t> xShape = {16};
    std::vector<int64_t> minMaxShape = {1};
    ADD_INPUT(1, x, inDtype, xShape, true, -8.0f, 8.0f, 0.0f);
    ADD_INPUT(2, min, inDtype, minMaxShape, false, 0.0f, 0.0f, -6.0f);
    ADD_INPUT(3, max, inDtype, minMaxShape, false, 0.0f, 0.0f, 6.0f);
...
int main(int argc, char* argv[])
{
    ...
    for (int i = 0; i < output_num; i++) {
        ...
        LOG_PRINT("\n========== fake_quant_with_min_max_vars result ==========\n");
        LOG_PRINT("x            y\n");
