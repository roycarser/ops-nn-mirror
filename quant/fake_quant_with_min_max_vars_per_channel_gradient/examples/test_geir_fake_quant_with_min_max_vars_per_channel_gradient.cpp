...
/*!
 * \file test_geir_fake_quant_with_min_max_vars_per_channel_gradient.cpp
 * \brief
 */
...
#include "../op_graph/fake_quant_with_min_max_vars_per_channel_gradient_proto.h"
...
#define ADD_INPUT(intputIndex, intputName, intputDtype, inputShape, useRandom, minVal, maxVal, fixedValue)          \
    ...
    fakeQuantGradOp.set_input_##intputName(placeholder##intputIndex);                                               \
    inputs.push_back(placeholder##intputIndex)

#define ADD_INPUT_ATTR(attrName, attrValue) fakeQuantGradOp.set_attr_##attrName(attrValue)

#define ADD_OUTPUT(outputIndex, outputName, outputDtype, outputShape)                                        \
    TensorDesc outputName##outputIndex##_desc_ = TensorDesc(ge::Shape(outputShape), FORMAT_ND, outputDtype); \
    fakeQuantGradOp.update_output_desc_##outputName(outputName##outputIndex##_desc_)
...
{
    Status ret = SUCCESS;
    auto fakeQuantGradOp = op::FakeQuantWithMinMaxVarsPerChannelGradient("fakeQuantWithMinMaxVarsPerChannelGradient1");

    std::vector<int64_t> shape = {4, 4};
    std::vector<int64_t> channelShape = {4};
    ADD_INPUT(1, gradients, inDtype, shape, true, -1.0f, 1.0f, 0.0f);
    ADD_INPUT(2, x, inDtype, shape, true, -8.0f, 8.0f, 0.0f);
    ADD_INPUT(3, min, inDtype, channelShape, false, 0.0f, 0.0f, -6.0f);
    ADD_INPUT(4, max, inDtype, channelShape, false, 0.0f, 0.0f, 6.0f);

    ADD_INPUT_ATTR(num_bits, 8);
    ADD_INPUT_ATTR(narrow_range, false);

    ADD_OUTPUT(1, backprops_wrt_x, inDtype, shape);
    ADD_OUTPUT(2, backprops_wrt_min, inDtype, channelShape);
    ADD_OUTPUT(3, backprops_wrt_max, inDtype, channelShape);

    outputs.push_back(fakeQuantGradOp);
    return SUCCESS;
}
...
int main(int argc, char* argv[])
{
    ...
    for (int i = 0; i < output_num; i++) {
        ...
        if (i == 0) {
            float* gradData = (float*)input[0].GetData();
            float* xData = (float*)input[1].GetData();
            LOG_PRINT("\n========== fake_quant_with_min_max_vars_per_channel_gradient result ==========\n");
            LOG_PRINT("gradients    x            backprops_wrt_x\n");
            for (int64_t j = 0; j < output_shape; j++) {
                LOG_PRINT("%f    %f    %f\n", gradData[j], xData[j], resultData[j]);
            }
            LOG_PRINT("===================================================================\n");
        } else {
            LOG_PRINT("backprops_wrt_%s:\n", (i == 1 ? "min" : "max"));
            for (int64_t j = 0; j < output_shape; j++) {
                LOG_PRINT("[%ld] %f\n", j, resultData[j]);
            }
        }
