...
每个算子分为[两段式接口](../../../docs/zh/context/two_phase_api.md)，必须先调用“aclnnDynamicQuantV2GetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnDynamicQuantV2”接口执行计算。
...
- **参数说明：**
  ...
  <!-- npu="A3,910b" id7 -->
  - <term>Atlas A2 训练系列产品/Atlas A2 推理系列产品</term>、<term>Atlas A3 训练系列产品/Atlas A3 推理系列产品</term>：出参`yOut`的数据类型仅支持INT4、INT8、INT32。
  <!-- end id7 -->
  <!-- npu="910,310p" id8 -->
