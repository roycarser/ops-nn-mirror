...
public:
    ...
    __aicore__ inline DequantSwigluQuantBaseStatic(TPipe* pipe) { pipe_ = pipe; };
...
template <typename TActScale, typename TQuantScale, typename TGroup, typename TBias, typename TXtype, typename TYtype>
__aicore__ inline void DequantSwigluQuantBaseStatic<TActScale, TQuantScale, TGroup, TBias, TXtype,
                                                    TYtype>::ProcessSingleGroupPerCore(int64_t groupIdx,
                                                                                       int64_t xDimPerCore,
                                                                                       int64_t coreDimxOffset)
{
    ...
    if constexpr (hasQuantScale_) {
        ...
        DataCopyPad(inScaleLocal[xUbAlignB32_ * 2], quantScaleGm_[groupIdx * quantUbFactorDimy],
                    dataCopyQuantScaleParams, padParams);
        if (hasQuantOffset_) {
            DataCopyPad(inScaleLocal[xUbAlignB32_ * 3], quantOffsetGm_[groupIdx * quantUbFactorDimy],
                        dataCopyQuantScaleParams, padParams);
        }
    }
    ...
    // UbFactorDimX：ub每次能处理的行数，ubDimxLoop:
    // 每个核上需要处理几行数据，然后在单个核上，ub处理这几行数据需要几次循环
    ...
    for (uint64_t i = 0; i < ubDimxLoop; i++) {
        // 核内循环每次计算时的起始地址：当前核计算时的起始地址 + 第i次核内循环要处理的起始地址 +
        // 已处理过的行偏移(历史已处理过的总行数)
        ...
        if (tl_->quantIsOne == 1 && hasQuantOffset_) {
            VF_CALL<StaticQuantWithQuantOffsetOne>(tmpXPtr, qScalePtr, qOffsetPtr, tmpXPtr, width, xDimPerLoop,
                                                   repeatTimes, sizePerRepeat, xTypeUbAlignB32_,
                                                   static_cast<uint16_t>(tl_->quantIsOne), hasQuantOffset_);
            ...
            VF_CALL<StaticQuantWithOne>(tmpXPtr, qScalePtr, qOffsetPtr, tmpXPtr, width, xDimPerLoop, repeatTimes,
                                        sizePerRepeat, xTypeUbAlignB32_, static_cast<uint16_t>(tl_->quantIsOne),
                                        hasQuantOffset_);
        } else if (tl_->quantIsOne != 1 && hasQuantOffset_) {
            VF_CALL<StaticQuantWithQuantOffset>(tmpXPtr, qScalePtr, qOffsetPtr, tmpXPtr, width, xDimPerLoop,
                                                repeatTimes, sizePerRepeat, xTypeUbAlignB32_,
                                                static_cast<uint16_t>(tl_->quantIsOne), hasQuantOffset_);
        } else if (tl_->quantIsOne != 1 && !hasQuantOffset_) {
        ...
        VF_CALL<CastY<TXtype, TYtype, ifYFloat8e4m3Index_, ifYFloat8e5m2Index_, ifYFloat4e2m1Index_,
                      ifYFloat4e1m2Index_, ifYHiFloat8Index_>>(tmpXPtr, yPtr, yFp4Ptr, tl_->UbFactorDimy, xDimPerLoop,
                                                               repeatTimes, sizePerRepeat, roundMode_, xTypeUbAlignB32_,
                                                               yUbAlignB8_, yUbAlignB4_);
...
template <typename TActScale, typename TQuantScale, typename TGroup, typename TBias, typename TXtype, typename TYtype>
__aicore__ inline void DequantSwigluQuantBaseStatic<TActScale, TQuantScale, TGroup, TBias, TXtype,
                                                    TYtype>::ProcessSingleGroup(int64_t groupIdx, int64_t realDimx,
                                                                                int64_t groupOffset)
{
