...
ge::graphStatus DequantSwigluQuantDskTiling::CheckBias()
{
    ...
    if (biasShapePtr != nullptr) {
        ...
        OP_CHECK_IF(CheckScaleShapeWithDim(BIAS_INDEX, inDimy_, "bias") != ge::GRAPH_SUCCESS,
                    OP_LOGE(context_->GetNodeName(), "bias shape check failed."), return ge::GRAPH_FAILED);
    } else {
        hasBias_ = false;
    }
    ...
    if (biasPtr != nullptr && hasBias_ == true) {
        ...
        bool dtypeInValid = (biasDtype != ge::DT_INT32 && biasDtype != ge::DT_FLOAT && biasDtype != ge::DT_FLOAT16 &&
                             biasDtype != ge::DT_BF16);
        OP_CHECK_IF(dtypeInValid,
                    OP_LOGE_FOR_INVALID_DTYPE(context_->GetNodeName(), "bias",
                                              ge::TypeUtils::DataTypeToSerialString(biasDtype).c_str(),
                                              "bf16, fp16, float or int32"),
                    return ge::GRAPH_FAILED);
...
ge::graphStatus DequantSwigluQuantDskTiling::CheckWeightScale()
{
    ...
    OP_CHECK_IF(CheckScaleShapeWithDim(WEIGHT_SCALE_INDEX, inDimy_, "weight_scale") != ge::GRAPH_SUCCESS,
                OP_LOGE(context_->GetNodeName(), "weight scale shape check failed."), return ge::GRAPH_FAILED);
...
ge::graphStatus DequantSwigluQuantDskTiling::CheckActivationScale()
{
    ...
    if (CheckOptionalShapeExisting(activationScaleShapePtr)) {
        ...
        OP_CHECK_IF(activationScaleNum != inDimx_,
...
ge::graphStatus DequantSwigluQuantDskTiling::CheckForDequant()
{
    ...
    if (xDtype == ge::DT_INT32) {
        OP_CHECK_IF(CheckWeightScale() != ge::GRAPH_SUCCESS,
                    OP_LOGE(context_->GetNodeName(), "weight scale check failed."), return ge::GRAPH_FAILED);
        OP_CHECK_IF(CheckActivationScale() != ge::GRAPH_SUCCESS,
                    OP_LOGE(context_->GetNodeName(), "activation scale check failed."), return ge::GRAPH_FAILED);
        OP_CHECK_IF(CheckBias() != ge::GRAPH_SUCCESS, OP_LOGE(context_->GetNodeName(), "bias check failed."),
                    return ge::GRAPH_FAILED);
    }
...
ge::graphStatus DequantSwigluQuantDskTiling::CheckForDynamicQuant()
{
    ...
    OP_CHECK_IF(offsetPtr != nullptr,
                OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(context_->GetNodeName(), "quant_offset", "not None",
                                                      "Quant_offset must be None in dynamic quant mode (quant_mode=1)"),
                return ge::GRAPH_FAILED);

    OP_CHECK_IF(CheckScaleShapeWithDim(QUANT_SCALE_INDEX, outDimy_, "quant_scale") != ge::GRAPH_SUCCESS,
                OP_LOGE(context_->GetNodeName(), "quant scale shape check failed."), return ge::GRAPH_FAILED);
    return ge::GRAPH_SUCCESS;
}
...
ge::graphStatus DequantSwigluQuantDskTiling::CheckForStaticQuant()
{
    ...
    OP_CHECK_IF(
        ...
        OP_LOGE_FOR_INVALID_DTYPES_WITH_REASON(context_->GetNodeName(), "quant_offset and quant_scale",
                                               (ge::TypeUtils::DataTypeToSerialString(quantOffsetDtype) + " and " +
                                                ge::TypeUtils::DataTypeToSerialString(quantScaleDtype))
                                                   .c_str(),
    ...
    OP_CHECK_IF(CheckStaticQuantShape(QUANT_OFFSET_INDEX, quantOffsetColLen, "quant_offset") != ge::GRAPH_SUCCESS,
                OP_LOGE(context_->GetNodeName(), "quant offset shape check failed."), return ge::GRAPH_FAILED);
...
ge::graphStatus DequantSwigluQuantDskTiling::CheckForQuant()
{
    ...
    OP_CHECK_IF(CheckQuantScaleDtype() != ge::GRAPH_SUCCESS,
                OP_LOGE(context_->GetNodeName(), "Check QuantScale Dtype failed."), return ge::GRAPH_FAILED);
    ...
    if (quantMode_ == QUANT_MODE_DYNAMIC) {
        OP_CHECK_IF(CheckForDynamicQuant() != ge::GRAPH_SUCCESS,
                    OP_LOGE(context_->GetNodeName(), "Check For Dynamic Quant failed."), return ge::GRAPH_FAILED);
    }
    // // check quant offset and quant scale shape in static scenario
    else {
        OP_CHECK_IF(CheckForStaticQuant() != ge::GRAPH_SUCCESS,
                    OP_LOGE(context_->GetNodeName(), "Check For Static Quant failed."), return ge::GRAPH_FAILED);
    }
...
ge::graphStatus DequantSwigluQuantDskTiling::GetAttr()
{
    ...
    OP_CHECK_IF(it == SUPPORT_QUANT_MODE.end(),
                OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(context_->GetNodeName(), "quant_mode", quantMode.c_str(),
                                                      "quant_mode only support dynamic(1) and static(0) currently"),
                return ge::GRAPH_FAILED);
...
ge::graphStatus DequantSwigluQuantDskTiling::CheckScaleShapeWithDim(const int64_t scaleInputIdx,
                                                                    const int64_t expectDim, const char* paramName)
{
    ...
    OP_CHECK_IF(
        ...
        OP_LOGE_FOR_INVALID_SHAPEDIM(context_->GetNodeName(), paramName, std::to_string(scaleShape.GetDimNum()).c_str(),
                                     "greater than or equal to 1"),
        return ge::GRAPH_FAILED);
    OP_CHECK_IF(scaleShape.GetDim(scaleShape.GetDimNum() - 1) != expectDim,
                OP_LOGE_FOR_INVALID_SHAPE(context_->GetNodeName(), paramName, Ops::Base::ToString(scaleShape).c_str(),
                                          std::to_string(expectDim).c_str()),
                return ge::GRAPH_FAILED);
    if (groupNum_ > 1) {
        // check with group index
        OP_CHECK_IF(scaleShape.GetDimNum() != DIM_SIZE_2,
        ...
        OP_CHECK_IF(groupNumFromScale != 1,
                                                                   ...
                                                                   const char* paramName)
{
    ...
    OP_CHECK_IF(
        quantShape.GetDimNum() < 1,
        OP_LOGE_FOR_INVALID_SHAPEDIM(context_->GetNodeName(), paramName, std::to_string(quantShape.GetDimNum()).c_str(),
                                     "greater than or equal to 1"),
        return ge::GRAPH_FAILED);
    ...
    if (quantShape.GetDimNum() == 1) {
        OP_CHECK_IF(
            colLen != groupNum_,
            OP_LOGE_FOR_INVALID_SHAPE(context_->GetNodeName(), paramName, Ops::Base::ToString(quantShape).c_str(),
                                      ("[" + std::to_string(groupNum_) + ", ] or [" + std::to_string(groupNum_) + ", " +
        ...
        OP_CHECK_IF(
            colLen != outDimy_ || quantShape.GetDim(0) != groupNum_,
            OP_LOGE_FOR_INVALID_SHAPE(context_->GetNodeName(), paramName, Ops::Base::ToString(quantShape).c_str(),
                                      ("[" + std::to_string(groupNum_) + ", ] or [" + std::to_string(groupNum_) + ", " +
                                       std::to_string(outDimy_) + "]")
                                          .c_str()),
...
ge::graphStatus DequantSwigluQuantDskTiling::GetShapeAttrsInfo() { return ge::GRAPH_SUCCESS; }
...
ge::graphStatus DequantSwigluQuantDskTiling::CheckIllegalParam()
{
    ...
    if (hasBias_) {
        OP_CHECK_IF(
            speGroupType_ == true,
            OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(context_->GetNodeName(), "group_index", speGroupType_ ? "2D" : "1D",
...
ge::graphStatus DequantSwigluQuantDskTiling::GetShapeAttrsInfoInner()
{
    ...
    OP_CHECK_IF(inDimy_ % (BLOCK_SIZE * SWI_FACTOR) != 0,
                OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(context_->GetNodeName(), "x", std::to_string(inDimy_).c_str(),
    ...
    OP_CHECK_IF(CheckXAndGroupIndexDtype() != ge::GRAPH_SUCCESS,
                OP_LOGE(context_->GetNodeName(), "dtype check failed."), return ge::GRAPH_FAILED);

    OP_CHECK_IF(GetAttr() != ge::GRAPH_SUCCESS, OP_LOGE(context_->GetNodeName(), "get attr failed."),
                return ge::GRAPH_FAILED);

    OP_CHECK_IF(CheckForDequant() != ge::GRAPH_SUCCESS, OP_LOGE(context_->GetNodeName(), "check for dequant failed."),
                return ge::GRAPH_FAILED);

    OP_CHECK_IF(CheckForQuant() != ge::GRAPH_SUCCESS, OP_LOGE(context_->GetNodeName(), "check for quant failed."),
                return ge::GRAPH_FAILED);

    OP_CHECK_IF(CheckIllegalParam() != ge::GRAPH_SUCCESS,
                OP_LOGE(context_->GetNodeName(), "check illegal param failed."), return ge::GRAPH_FAILED);
...
void DequantSwigluQuantDskTiling::CountTilingKey()
{
    ...
    bool cond1 = speGroupType_ && (groupNum_ >= CUT_GROUP_LARGE_THAN_64) &&
                 (inDimx_ / groupNum_ <= EACH_GROUP_TOKEN_LESS_THAN);
    bool cond2 = !speGroupType_ && (groupNum_ >= CUT_GROUP_LARGE_THAN_32) &&
                 (inDimx_ / groupNum_ <= EACH_GROUP_TOKEN_LESS_THAN) && !tilingData_.get_biasDtype() &&
                 !tilingData_.get_quantScaleDtype() && (xDtype == ge::DT_INT32);
    if (cond1 || cond2) {
...
ge::graphStatus DequantSwigluQuantDskTiling::CountMaxDim(int64_t& ubFactorDimx)
{
    ...
    */
    ...
    int64_t SweiGLUBufferX = swigluMode_ == 1 ? outDimy_ * static_cast<int64_t>(sizeof(int8_t)) +
                                                    outDimy_ * static_cast<int64_t>(sizeof(int32_t)) :
                                                0;
    ...
    int64_t numerator = static_cast<int64_t>(ubSize_) - UB_RESERVE - BLOCK_SIZE - db * BLOCK_SIZE -
                        static_cast<int64_t>(sizeof(float));
    int64_t denominator = 5 * static_cast<int64_t>(sizeof(float)) +
                          db * SWI_FACTOR * static_cast<int64_t>(sizeof(float)) + static_cast<int64_t>(sizeof(int8_t)) +
                          biasBufferY + SweiGLUBufferY + quantOffsetSpace;
    ...
    OP_CHECK_IF(inDimy_ > maxInDimy,
                OP_LOGE_FOR_INVALID_SHAPESIZE(context_->GetNodeName(), "x", std::to_string(inDimy_).c_str(),
    ...
    numerator = static_cast<int64_t>(ubSize_) - UB_RESERVE - outDimy_ * static_cast<int64_t>(sizeof(float)) -
                BLOCK_SIZE - SWI_FACTOR * outDimy_ * static_cast<int64_t>(sizeof(float)) - biasBufferX -
                quantOffsetSpace;

    denominator = db * (outDimy_ * SWI_FACTOR + BLOCK_ELEM) * static_cast<int64_t>(sizeof(float)) +
                  outDimy_ * static_cast<int64_t>(sizeof(int8_t)) + static_cast<int64_t>(sizeof(float)) +
...
ge::graphStatus DequantSwigluQuantDskTiling::DoOpTiling()
{
    ...
    OP_CHECK_IF(CountMaxDim(ubFactorDimx) != ge::GRAPH_SUCCESS,
                OP_LOGE(context_->GetNodeName(), "Count MaxDim failed."), return ge::GRAPH_FAILED);
...
ge::graphStatus DequantSwigluQuantDskTiling::DoLibApiTiling() { return ge::GRAPH_SUCCESS; }

uint64_t DequantSwigluQuantDskTiling::GetTilingKey() const { return tilingKey_; }
