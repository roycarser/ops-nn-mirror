...
#ifndef SORTED_SPARSE_SEGMENT_MEAN_GRAD_SIMT_LARGE_INNER_H
#define SORTED_SPARSE_SEGMENT_MEAN_GRAD_SIMT_LARGE_INNER_H
...
constexpr uint32_t MAX_SIMPLE_THREAD_NUM = 512;
#else
constexpr uint32_t MAX_THREAD_NUM = 512;
constexpr uint32_t MAX_SIMPLE_THREAD_NUM = 1024;
#endif
                                                                                           ...
                                                                                           const SortedSparseSegmentMeanGradSimtTilingData* tilingData) 
{
    ...
    newBlockIdx_ = blockIdx_;

    if (tilingData_->innerCore > 1) {
        newBlockIdx_ = blockIdx_ / tilingData_->innerCore;
    }
    if (newBlockIdx_ < static_cast<uint32_t>(tilingData_->resIndicesNum)) {
...
__aicore__ inline void SortedSparseSegmentMeanGradSimtLargeInner<X_T, INDICES_T, LOCATION_T, SEGMENTIDS_T, OUTTER_T, INNER_T>::Process()
{
    ...
    Duplicate<float>(tmpLocal, 0, MAX_UB_FLOAT_NUM);
    ...
    INNER_T processCols = innerSize;
    INNER_T srcColOffset = 0;
    bool gtMaxUbNum = innerSize > MAX_UB_FLOAT_NUM;
    bool gtThreadNum = false;
    if (tilingData_->innerCore > 1) {
        uint32_t blockInnerCoreIdx = blockIdx_ % tilingData_->innerCore;
        if (blockInnerCoreIdx < tilingData_->resCoreInnerSize) {
            processCols = static_cast<INNER_T>(tilingData_->perCoreInnerSize + 1);
    ...
    if (gtMaxUbNum) {
        AscendC::Simt::VF_CALL<SimtLargeInnerComputer<X_T, LOCATION_T, SEGMENTIDS_T, OUTTER_T, INNER_T>>(Simt::Dim3{threadNumX, threadNumY}, indicesOffsetBase_, curCoreIndices_, threadNumY, innerSize,
        ...
        AscendC::Simt::VF_CALL<SimtLargeUBInnerComputer<X_T, LOCATION_T, SEGMENTIDS_T, OUTTER_T, INNER_T>>(Simt::Dim3{threadNumX, threadNumY}, indicesOffsetBase_, curCoreIndices_, innerSize,
                                                                    segmentNum_, (__gm__ X_T*) (xGm_.GetPhyAddr()), (__gm__ volatile X_T*) (yGm_.GetPhyAddr()),
                                                                   (__gm__ OUTTER_T*) (workspaceIndicesOffset_.GetPhyAddr(2 * (segmentNum_ + 1))), (__gm__ SEGMENTIDS_T*) (segmentIdsGm_.GetPhyAddr()),
                                                                   (__gm__ LOCATION_T*) (locationGm_.GetPhyAddr()), (__gm__ float*) (workspaceWeight_.GetPhyAddr(segmentNum_ + 1)),
        ...
        AscendC::Simt::VF_CALL<SimtLargeMinInnerComputer<X_T, LOCATION_T, SEGMENTIDS_T, OUTTER_T, INNER_T>>(Simt::Dim3{threadNumX, threadNumY}, indicesOffsetBase_, curCoreIndices_, innerSize,
                                                                    segmentNum_, (__gm__ X_T*) (xGm_.GetPhyAddr()), (__gm__ volatile X_T*) (yGm_.GetPhyAddr()),
                                                                   (__gm__ OUTTER_T*) (workspaceIndicesOffset_.GetPhyAddr(2 * (segmentNum_ + 1))), (__gm__ SEGMENTIDS_T*) (segmentIdsGm_.GetPhyAddr()),
                                                                   (__gm__ LOCATION_T*) (locationGm_.GetPhyAddr()), (__gm__ float*) (workspaceWeight_.GetPhyAddr(segmentNum_ + 1)), 
