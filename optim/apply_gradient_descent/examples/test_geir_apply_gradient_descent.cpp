...
/*!
 * \file test_geir_apply_gradient_descent.cpp
 * \brief
 */
...
#define ADD_INPUT(intputIndex, intputName, intputDtype, inputShape)                                             \
    ...
    applyGradientDescent1.set_input_##intputName(placeholder##intputIndex);                                             \
    inputs.push_back(placeholder##intputIndex)

#define ADD_INPUT_ATTR(attrName, attrValue) applyGradientDescent1.set_attr_##attrName(attrValue)
...
#define ADD_CONST_INPUT(intputIndex, intputName, intputDtype, inputShape)                                   \
    ...
    graph.AddOp(placeholder##intputIndex);                                                                  \
    applyGradientDescent1.set_input_##intputName(placeholder##intputIndex);                                         \
    applyGradientDescent1.update_input_desc_##intputName(placeholder##intputIndex##_desc);                          \
    inputs.push_back(placeholder##intputIndex);

#define ADD_OUTPUT(outputIndex, outputName, outputDtype, outputShape)                                        \
    TensorDesc outputName##outputIndex##_desc_ = TensorDesc(ge::Shape(outputShape), FORMAT_ND, outputDtype); \
    applyGradientDescent1.update_output_desc_##outputName(outputName##outputIndex##_desc)
    ...
    Graph& graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto applyGradientDescent1 = op::ApplyGradientDescent("applyGradientDescent1");
    std::vector<int64_t> varShape = {(0)};
    std::vector<int64_t> alphaShape = {(1)};
    std::vector<int64_t> deltaShape = {(0)};
    ADD_INPUT(1, var, inDtype, varShape);
    ADD_INPUT(2, alpha, inDtype, alphaShape);
    ADD_INPUT(3, delta, inDtype, deltaShape);

    outputs.push_back(applyGradientDescent1);
    // 添加完毕
    return SUCCESS;
}
