...
def compare_data(golden_file_lists, output_file_lists, d_type):
    ...
    if d_type == "float16":
        ...
        np_dtype = np.int32
        precision = 0
    elif d_type in ["int16", "int8", "uint8"]:
        np_dtype = (
            np.int16 if d_type == "int16" else np.int8 if d_type == "int8" else np.uint8
        )
        precision = 0
    elif d_type == "float32":
        precision = 1 / 10000
    ...
    for gold, out in zip(golden_file_lists, output_file_lists):
        ...
        if d_type == "bfloat16_t":
            tmp_out_compare = tmp_out.astype(np.float32)
            tmp_gold_compare = tmp_gold.astype(np.float32)
        else:
            tmp_out_compare = tmp_out
            tmp_gold_compare = tmp_gold
        diff_res = np.isclose(tmp_out_compare, tmp_gold_compare, precision, 0, True)
