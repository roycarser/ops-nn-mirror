                                       ...
                                       DtypeSize dtypeSizes, bool hasScale)
{
    ...
    uint64_t biasUsedL1Size = tilingData.hasBias ? ConvAlignB(nBL1min * dtypeSizes.biasDtypeSize, C0_SIZE) : 0;
    ...
    uint64_t hiAL1min = InferHiL1(hoAL1min, tilingData.orgHi, tilingData.kernelH, tilingData.dilationH,
                                  tilingData.strideH);
                                        ...
                                        DtypeSize dtypeSizes, bool hasScale)
{
    ...
    uint64_t nBL1min = N0_SIZE;
    uint64_t biasUsedL1Size = tilingData.hasBias ? ConvAlignB(nBL1min * biasDtypeSize, C0_SIZE) : 0;
...
bool CheckL1SizeLimitsInHWsplitMode(TilingParam& tilingData, DtypeSize dtypeSizes, bool hasScale)
{
    ...
    uint64_t wiAL1min = InferWiL1(woAL1min, tilingData.orgWi, tilingData.kernelW, tilingData.dilationW,
                                  tilingData.strideW);
    uint64_t usdL1SizeUnderMinHWtiling = CalcMinUsedL1SizeInHWsplitMode(tilingData, kParmas, wiAL1min, dtypeSizes,
                                                                        hasScale);
...
bool CheckL1SizeLimitsInMsplitMode(TilingParam& tilingData, DtypeSize dtypeSizes, bool hasScale)
{
    ...
    uint64_t usdL1SizeUnderMinMtiling = CalcMinUsedL1SizeInMsplitMode(
        tilingData, k0, tilingData.kernelH * tilingData.kernelW * k0, dtypeSizes, hasScale);
...
bool CheckC04L1SizeLimitsInMsplitMode(TilingParam& tilingData, DtypeSize dtypeSizes, bool hasScale)
{
    ...
    uint64_t c04UsdL1SizeUnderMinMtiling = CalcMinUsedL1SizeInMsplitMode(
        tilingData, C04_SIZE, ConvAlignB(C04_SIZE * tilingData.kernelH * tilingData.kernelW, k0), dtypeSizes, hasScale);
...
uint64_t CalcUsdL1SizeMode(TilingParam& tilingData, DtypeSize dtypeSize, int8_t pbAL1, int8_t pbBL1)
{
...
uint64_t CalcUsdL1SizeHWode(TilingParam& tilingData, DtypeSize dtypeSize, int8_t pbAL1, bool isC04Flag)
{
    uint64_t hiL1 = InferHiL1(tilingData.hoL1, tilingData.orgHi, tilingData.kernelH, tilingData.dilationH,
                              tilingData.strideH);
    uint64_t wiL1 = InferWiL1(tilingData.woL1, tilingData.orgWi, tilingData.kernelW, tilingData.dilationW,
                              tilingData.strideW);
    uint64_t al1Size = hiL1 * wiL1 * (tilingData.kAL1 / (tilingData.kernelH * tilingData.kernelW)) * (pbAL1 + 1) *
                       dtypeSize.fMapDtypeSize;
    if (isC04Flag) {
        al1Size = ConvCeilDiv(hiL1 * wiL1 * C04_SIZE * dtypeSize.fMapDtypeSize, C0_SIZE) * C0_SIZE * (pbAL1 + 1);
    }
    uint64_t bl1Size = tilingData.nBL1 * tilingData.kBL1 * dtypeSize.weightDtypeSize;
    return al1Size + bl1Size;
}

uint64_t CalcUsdL1Size(TilingParam& tilingData, DtypeSize dtypeSize, int64_t padCompensationValue, bool hasQuantScale,
                       bool isC04Flag)
{
...
void GetInitBasicBlockMN(TilingParam& tilingData, uint64_t& basicBlockM, uint64_t& basicBlockN)
{
    ...
    } else if (tilingData.cout > BASICBLOCK_BOUNDARY_VALUE_64 && tilingData.cout <= BASICBLOCK_BOUNDARY_VALUE_128) {
...
bool CmpFirstAdjustMnTile(TilingParam& tilingData, int64_t& mTile, int64_t& nTile, BasicParams basicParams,
                          vector<int64_t> pads)
{
    ...
    if (basicBlockM >= tilingData.wout * tilingData.hout) { // L1 can full load M direction
        ...
        hoMax = (maxhiL1 + padCompensationValue -
                 static_cast<int64_t>(tilingData.dilationH) * (static_cast<int64_t>(tilingData.kh) - 1) - 1) /
                    static_cast<int64_t>(tilingData.strideH) +
                1;
    } else {
...
void SelectMmodeAlgorithm(TilingParam& tilingData, bool& mBasicBlockModeFlag, DtypeSize dtypeSize, vector<int64_t> pads,
                          bool hasScale)
{
    uint32_t featuremapDtyeSize = dtypeSize.fMapDtypeSize;
    bool hasBias = tilingData.hasBias;
...
void CheckHWModeTilingDataValidForConv2d(TilingParam& tilingData, uint64_t k0, bool isC04Flag, bool isOptPreloadFlag)
{
    ...
    if (isC04Flag) {
        ...
        if (tilingData.kAL1 % tilingData.kL0 == 0 || tilingData.kAL1 == ConvCeilDiv(tilingData.singleCoreCi, k0) * k0 *
                                                                            tilingData.kernelH * tilingData.kernelW) {
            kAL1DivCheck = 1;
        }
...
bool CheckValidTilingDataPartOne(TilingParam& tilingData, DtypeSize dtypeSize, std::vector<int64_t> pads, bool hasScale,
                                 uint64_t tilingKey)
{
                                        ...
                                        bool isOptPreloadFlag)
{
    ...
    EXPECT_LE(tilingData.hoL0, ConvCeilDiv(std::min(MEM_SIZE_64K / (k0 * (pbAL0 + 1) * dtypeSize.fMapDtypeSize),
                                                    MEM_SIZE_256K / (N0_SIZE * (pbCL0 + 1) * mmadDtypeSize)),
                                           M0_SIZE) *
                                   M0_SIZE);
    EXPECT_LE(tilingData.nL0, ConvCeilDiv(std::min(MEM_SIZE_64K / (k0 * (pbBL0 + 1) * dtypeSize.weightDtypeSize),
                                                   MEM_SIZE_256K / (M0_SIZE * (pbCL0 + 1) * mmadDtypeSize)),
                                          N0_SIZE) *
                                  N0_SIZE);
...
void CheckValidTilingData(TilingParam& tilingData, DtypeSize dtypeSize, std::vector<int64_t> pads, bool hasScale,
                          uint64_t tilingKey)
{
...
void GetOriPadFromPadModeConv2D(PadModeParams padModeParams, uint32_t& padu, uint32_t& padd, uint32_t& padl,
                                uint32_t& padr)
{
...
void ExtendConv2DTestCase(vector<int64_t> fmShape, vector<int64_t> weightShape, vector<uint32_t> pads,
                          vector<uint32_t> strides, vector<uint32_t> dilations, ge::DataType inDataType,
                          ge::DataType out0DataType, ge::DataType out1DataType, bool isHasBias = true,
                          bool isHasScale0 = false, bool isHasScale1 = false, bool isHasReluWeight0 = false,
                          bool isHasReluWeight1 = false, bool isHasClipValue0 = false, bool isHasClipValue1 = false,
                          bool enableRelu0 = false, bool enableRelu1 = false, bool dualOutput = false,
                          bool enableHf32Mode = false, uint32_t groups = 1, string padMode = "SPECIFIC",
                          uint8_t errorCaseStatus = 0, string format = "NCHW", string round_mode = "rint")
{
    ...
    PadModeParams padModeParams = {padMode, strideH, strideW, dilationH, dilationW, batch, cin, hi, wi, cout, kH, kW};
    ...
    ge::DataType biasDataType = (inDataType == ge::DT_INT8)                                           ? ge::DT_INT32 :
                                (inDataType == ge::DT_HIFLOAT8 || inDataType == ge::DT_FLOAT8_E4M3FN) ? ge::DT_FLOAT :
                                                                                                        inDataType;
...
{
    ExtendConv2DTestCase({1, 256, 64, 64}, {256, 3, 3}, {0, 0, 0, 0}, {1, 1}, {1, 1}, ge::DT_HIFLOAT8, ge::DT_HIFLOAT8,
                         ge::DT_HIFLOAT8,                 // inDataType, out0DataType, out1DataType
                         true, true, false,               // isHasBias, isHasScale0, isHasScale1,
                         false, false,                    // isHasReluWeight0, isHasReluWeight1
                         false, false,                    // isHasClipValue0, isHasClipValue1
                         false, false, false,             // enableRelu0, enableRelu1, dualOutput
                         false, 1,                        // enableHf32Mode, groups
                         "SPECIFIC", 0, "NCHW", "round"); // errorCaseStatus, format, round_mode
}

TEST_F(ExtendConv2dTiling, run_ExtendConv2D_case_004)
{
    ExtendConv2DTestCase({1, 256, 64, 64}, {256, 3, 3}, {0, 0, 0, 0}, {1, 1}, {1, 1}, ge::DT_FLOAT8_E4M3FN,
                         ge::DT_FLOAT8_E4M3FN, ge::DT_FLOAT8_E4M3FN, // inDataType, out0DataType, out1DataType
                         true, true, false,                          // isHasBias, isHasScale0, isHasScale1,
                         false, false,                               // isHasReluWeight0, isHasReluWeight1
                         false, false,                               // isHasClipValue0, isHasClipValue1
                         false, false, false,                        // enableRelu0, enableRelu1, dualOutput
                         false, 1,                                   // enableHf32Mode, groups
                         "SPECIFIC", 0, "NCHW", "rint");             // errorCaseStatus, format, round_mode
}
...
TEST_F(ExtendConv2dTiling, run_ExtendConv2D_case_005)
{
    ExtendConv2DTestCase({1, 16, 256, 256}, {32, 3, 3}, {2, 2, 2, 2}, {1, 1}, {1, 1}, ge::DT_INT8, ge::DT_FLOAT16,
                         ...
                         false, false, false,    // enableRelu0, enableRelu1, dualOutput
                         false, 1,               // enableHf32Mode, groups
                         "SPECIFIC", 0, "NHWC"); // errorCaseStatus, format
}

TEST_F(ExtendConv2dTiling, run_ExtendConv2D_case_006)
{ // extendconv2d cache tiling
    ExtendConv2DTestCase({1, 16, 256, 256}, {32, 3, 3}, {2, 2, 2, 2}, {1, 1}, {1, 1}, ge::DT_INT8, ge::DT_FLOAT16,
                         ge::DT_FLOAT16,         // inDataType, out0DataType, out1DataType
                         true, true, false,      // isHasBias, isHasScale0, isHasScale1,
                         false, false,           // isHasReluWeight0, isHasReluWeight1
...
{
    ExtendConv2DTestCase({1, 256, 64, 64}, {256, 3, 3}, {0, 0, 0, 0}, {1, 1}, {1, 1}, ge::DT_HIFLOAT8, ge::DT_FLOAT,
                         ge::DT_FLOAT,                    // inDataType, out0DataType, out1DataType
                         true, true, false,               // isHasBias, isHasScale0, isHasScale1,
                         false, false,                    // isHasReluWeight0, isHasReluWeight1
                         false, false,                    // isHasClipValue0, isHasClipValue1
                         false, false, false,             // enableRelu0, enableRelu1, dualOutput
...
{
    ExtendConv2DTestCase({1, 256, 64, 64}, {256, 3, 3}, {0, 0, 0, 0}, {1, 1}, {1, 1}, ge::DT_HIFLOAT8, ge::DT_BF16,
                         ge::DT_BF16,                     // inDataType, out0DataType, out1DataType
                         true, true, false,               // isHasBias, isHasScale0, isHasScale1,
                         false, false,                    // isHasReluWeight0, isHasReluWeight1
                         false, false,                    // isHasClipValue0, isHasClipValue1
                         false, false, false,             // enableRelu0, enableRelu1, dualOutput
...
{
    ExtendConv2DTestCase({1, 256, 64, 64}, {256, 3, 3}, {0, 0, 0, 0}, {1, 1}, {1, 1}, ge::DT_HIFLOAT8, ge::DT_FLOAT16,
                         ge::DT_FLOAT16,                  // inDataType, out0DataType, out1DataType
                         true, true, false,               // isHasBias, isHasScale0, isHasScale1,
                         false, false,                    // isHasReluWeight0, isHasReluWeight1
                         false, false,                    // isHasClipValue0, isHasClipValue1
...
{
    ExtendConv2DTestCase({1, 256, 64, 64}, {256, 3, 3}, {0, 0, 0, 0}, {1, 1}, {1, 1}, ge::DT_FLOAT8_E4M3FN,
                         ge::DT_FLOAT, ge::DT_FLOAT,     // inDataType, out0DataType, out1DataType
                         true, true, false,              // isHasBias, isHasScale0, isHasScale1,
                         false, false,                   // isHasReluWeight0, isHasReluWeight1
                         false, false,                   // isHasClipValue0, isHasClipValue1
                         false, false, false,            // enableRelu0, enableRelu1, dualOutput
...
{
    ExtendConv2DTestCase({1, 256, 64, 64}, {256, 3, 3}, {0, 0, 0, 0}, {1, 1}, {1, 1}, ge::DT_FLOAT8_E4M3FN, ge::DT_BF16,
                         ge::DT_BF16,                    // inDataType, out0DataType, out1DataType
                         true, true, false,              // isHasBias, isHasScale0, isHasScale1,
                         false, false,                   // isHasReluWeight0, isHasReluWeight1
                         false, false,                   // isHasClipValue0, isHasClipValue1
...
{
    ExtendConv2DTestCase({1, 256, 64, 64}, {256, 3, 3}, {0, 0, 0, 0}, {1, 1}, {1, 1}, ge::DT_FLOAT8_E4M3FN,
                         ge::DT_FLOAT16, ge::DT_FLOAT16, // inDataType, out0DataType, out1DataType
                         true, true, false,              // isHasBias, isHasScale0, isHasScale1,
                         false, false,                   // isHasReluWeight0, isHasReluWeight1
                         false, false,                   // isHasClipValue0, isHasClipValue1
                         false, false, false,            // enableRelu0, enableRelu1, dualOutput
...
{
    ExtendConv2DTestCase({4, 23, 1894, 16}, {253, 1, 2}, {0, 0, 0, 0}, {48, 7}, {1, 1}, ge::DT_INT8, ge::DT_FLOAT16,
                         ge::DT_FLOAT16,         // inDataType, out0DataType, out1DataType
                         true, true, false,      // isHasBias, isHasScale0, isHasScale1,
                         false, false,           // isHasReluWeight0, isHasReluWeight1
                         false, false,           // isHasClipValue0, isHasClipValue1
...
{
    ExtendConv2DTestCase({4, 4, 43765, 5}, {1024, 165, 4}, {146, 10, 1, 1}, {13, 2}, {4, 1}, ge::DT_FLOAT8_E4M3FN,
                         ge::DT_BF16, ge::DT_BF16,       // inDataType, out0DataType, out1DataType
                         true, true, false,              // isHasBias, isHasScale0, isHasScale1,
                         false, false,                   // isHasReluWeight0, isHasReluWeight1
                         false, false,                   // isHasClipValue0, isHasClipValue1
                         false, false, false,            // enableRelu0, enableRelu1, dualOutput
...
{
    ExtendConv2DTestCase({1, 16, 256, 256}, {32, 3, 3}, {2, 2, 2, 2}, {1, 1}, {1, 1}, ge::DT_INT8, ge::DT_INT8,
                         ge::DT_FLOAT16,         // inDataType, out0DataType, out1DataType
                         true, true, false,      // isHasBias, isHasScale0, isHasScale1,
                         false, false,           // isHasReluWeight0, isHasReluWeight1
                         false, false,           // isHasClipValue0, isHasClipValue1
...
{
    ExtendConv2DTestCase({1, 16, 256, 256}, {32, 3, 3}, {2, 2, 2, 2}, {1, 1}, {1, 1}, ge::DT_INT8, ge::DT_FLOAT16,
                         ge::DT_INT8,            // inDataType, out0DataType, out1DataType
                         true, true, true,       // isHasBias, isHasScale0, isHasScale1,
                         false, false,           // isHasReluWeight0, isHasReluWeight1
                         false, false,           // isHasClipValue0, isHasClipValue1
                         false, false, true,     // enableRelu0, enableRelu1, dualOutput
...
{
    ExtendConv2DTestCase({1, 16, 256, 256}, {32, 3, 3}, {2, 2, 2, 2}, {1, 1}, {1, 1}, ge::DT_FLOAT8_E4M3FN, ge::DT_INT8,
                         ge::DT_FLOAT16,         // inDataType, out0DataType, out1DataType
                         true, true, true,       // isHasBias, isHasScale0, isHasScale1,
                         false, false,           // isHasReluWeight0, isHasReluWeight1
                         false, false,           // isHasClipValue0, isHasClipValue1
...
{
    ExtendConv2DTestCase({1, 16, 256, 256}, {32, 3, 3}, {2, 2, 2, 2}, {1, 1}, {1, 1}, ge::DT_FLOAT8_E4M3FN,
                         ge::DT_FLOAT8_E4M3FN, ge::DT_FLOAT16, // inDataType, out0DataType, out1DataType
                         true, true, false,                    // isHasBias, isHasScale0, isHasScale1,
                         false, false,                         // isHasReluWeight0, isHasReluWeight1
                         false, false,                         // isHasClipValue0, isHasClipValue1
                         false, false, false,                  // enableRelu0, enableRelu1, dualOutput
                         false, 1,                             // enableHf32Mode, groups
                         "SPECIFIC", 1, "NHWC");               // errorCaseStatus, format
}

TEST_F(ExtendConv2dTiling, run_ExtendConv2D_case_nhwc_hif8_in_hif8_out_case)
{
    ExtendConv2DTestCase({1, 16, 256, 256}, {32, 3, 3}, {2, 2, 2, 2}, {1, 1}, {1, 1}, ge::DT_HIFLOAT8, ge::DT_HIFLOAT8,
                         ge::DT_FLOAT16,         // inDataType, out0DataType, out1DataType
                         true, true, false,      // isHasBias, isHasScale0, isHasScale1,
                         false, false,           // isHasReluWeight0, isHasReluWeight1
                         false, false,           // isHasClipValue0, isHasClipValue1
...
{
    ExtendConv2DTestCase({2, 16, 256, 256}, {32, 3, 3}, {2, 2, 2, 2}, {1, 1}, {1, 1}, ge::DT_INT8, ge::DT_FLOAT16,
                         ge::DT_FLOAT16,         // inDataType, out0DataType, out1DataType
                         true, true, true,       // isHasBias, isHasScale0, isHasScale1,
                         false, false,           // isHasReluWeight0, isHasReluWeight1
                         false, false,           // isHasClipValue0, isHasClipValue1
...
{
    ExtendConv2DTestCase({2, 16, 256, 256}, {32, 3, 3}, {2, 2, 2, 2}, {1, 1}, {1, 1}, ge::DT_FLOAT16, ge::DT_FLOAT16,
                         ge::DT_FLOAT16,         // inDataType, out0DataType, out1DataType
