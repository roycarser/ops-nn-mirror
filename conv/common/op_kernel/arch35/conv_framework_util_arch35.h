...
#define CONV_DECLARE_REG_IMPL(MEMBER, args...)                                                                     \
    namespace __ConvFramework {                                                                                    \
...
#define CONV_REG_IMPL(Config, Current, MEMBER)                    \
    ...
    struct __##MEMBER {                                           \
        using Type = typename Current::MEMBER<Intf, ImplType>;    \
    };                                                            \
    template <class T>                                            \
    struct __##MEMBER<true, T> {                                  \
        using Type = typename T::template MEMBER<Intf, ImplType>; \
    };                                                            \
...
#define CONV_DECLARE_CHECK_FUN(T, NAMESPACE)                                   \
    template <class... Ts>                                                     \
    static __aicore__ inline NAMESPACE::TypeFalse call(Intf* self, Ts... args) \
    {                                                                          \
        return (NAMESPACE::TypeFalse){0};                                      \
    }

// Check whether the fun T::call exists.
#define CONV_CHECK_FUN(T, NAMESPACE, ...) (!IsSameType<decltype(T::call(__VA_ARGS__)), NAMESPACE::TypeFalse>::value)
...
#define CONV_DECLARE_CHECK_MEMBER(MEMBER)                                \
    namespace __ConvFramework {                                          \
    template <typename T>                                                \
...
#define CONV_DECLARE_DEFINE_STRUCT(T, M, U)                                                                  \
    namespace __ConvFramework {                                                                              \
    template <typename T>                                                                                    \
    struct T##_##M {                                                                                         \
        U M;                                                                                                 \
...
#define CONV_DEFINE_STUCT(T, M) \
public                          \
    decltype(__ConvFramework::T##_##M##_checkdefine<T>())
