...
const std::vector<DataType> REDUCESUM_SUPPORTED_DTYPES = {DataType::DT_FLOAT16, DataType::DT_FLOAT, DataType::DT_BF16};
static bool IsInputSupportFp32Local()
{
    // 判断当前SOC版本是否支持Fp32输入
    auto curArch = GetCurrentPlatformInfo().GetCurNpuArch();
    if (curArch == NpuArch::DAV_2201 || Ops::NN::AclnnUtil::IsRegbase(curArch)) {
        OP_LOGD("The soc version is Ascend910B or ASCEND910_93 or ASCEND950, ConvolutionBackward support input tensor "
                "with Fp32");
        return true;
    }
    return false;
...
static bool CheckDeterministic(const int64_t deterministicValue, int groups)
{
    OP_CHECK(!((deterministicValue == 1) && (groups > 1)),
             OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(
                 ACLNN_CONVOLUTION_BACKWARD_NAME, "groups", (std::to_string(groups)).c_str(),
...
static bool IsPaddingValidFor3D(const aclTensor* weight, const ConvolutionBackwardParams& params)
{
    ...
    if (padH >= kernelDilationH || padW >= kernelDilationW) {
        OP_LOGD("Padding is too large for 3D branch: padH=%ld, padW=%ld, kernelH=%ld, kernelW=%ld, dilationH=%ld, "
                "dilationW=%ld",
...
static bool isConv3dDwV2Valid(const aclTensor* gradOutput, const aclTensor* input, const aclTensor* weight,
                              const ConvolutionBackwardParams& params)
{
                                                  ...
                                                  ConvolutionBackwardParams& params)
{
    ...
    if (params.cubeMathType == USE_FP16) {
        OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(
            ACLNN_CONVOLUTION_BACKWARD_NAME, "cubeMathType", std::to_string(params.cubeMathType).c_str(),
            "the value of cubeMathType must be 0 or 1 or 3,"
    ...
    if (promoteType == DataType::DT_FLOAT16 || promoteType == DataType::DT_BF16) {
        OP_LOGE_FOR_INVALID_DTYPES_WITH_REASON(
            ACLNN_CONVOLUTION_BACKWARD_NAME, "gradOutput, input, weight",
            FormatString("[%d,%d,%d]", inputTensor.gradOutput->GetDataType(), inputTensor.input->GetDataType(),
...
static const aclTensor* CallConv2DBackpropFilter(const aclTensor* input, const aclTensor* weight,
                                                 ...
                                                 aclOpExecutor* executor)
{
    if (useHf32) {
        ...
        return l0op::Conv2DBackpropFilterFp322Fp32(input, weight, gradOutput, stride, padding, dilation, groups,
                                                   executor);
    } else if (inputDtype == DataType::DT_BF16) {
        return l0op::Conv2DBackpropFilterBf162Fp32(input, weight, gradOutput, stride, padding, dilation, groups,
                                                   executor);
    } else {
        return l0op::Conv2DBackpropFilterFp162Fp32(input, weight, gradOutput, stride, padding, dilation, groups,
                                                   executor);
    }
}
...
static const aclTensor* CreateTensorView(const aclTensor* input, const op::Shape& newOriginalShape,
                                         const op::Shape& newStorageShape, const op::Strides& newViewStride,
                                         Format viewFormat, Format originalFormat, Format storageFormat,
                                         aclOpExecutor* executor)
{
    auto newTensor = executor->CreateView(input, newOriginalShape, newStorageShape, newViewStride,
                                          input->GetViewOffset());
...
static op::Format MergeFormat(op::Format formatA, op::Format formatB)
{
...
static const aclTensor* View5hdas6hd(const aclTensor* input, aclOpExecutor* executor)
{
    ...
    op::Strides newViewStride = op::Strides(
        {viewStride[0], viewStride[1], viewStride[1], viewStride[2], viewStride[3]});
    op::Shape newOriginalShape = op::Shape({originalShape[0], originalShape[1], 1, originalShape[2], originalShape[3]});
    op::Shape newStorageShape = op::Shape(
        {storageShape[0], 1, storageShape[1], storageShape[2], storageShape[3], storageShape[4]});

    return CreateTensorView(input, newOriginalShape, newStorageShape, newViewStride, Format::FORMAT_NCDHW,
                            Format::FORMAT_NCDHW, newStorageFormat, executor);
}
...
static const aclTensor* ViewFZasFZ3D(const aclTensor* input, aclOpExecutor* executor)
{
    ...
    op::Strides newViewStride = op::Strides(
        {viewStride[0], viewStride[1], viewStride[1], viewStride[2], viewStride[3]});
    op::Shape newOriginalShape = op::Shape({originalShape[0], originalShape[1], 1, originalShape[2], originalShape[3]});
    return CreateTensorView(input, newOriginalShape, storageShape, newViewStride, Format::FORMAT_NCDHW,
                            Format::FORMAT_NCDHW, newStorageFormat, executor);
}
...
static const aclTensor* ViewFZ3DasFZ(const aclTensor* input, aclOpExecutor* executor)
{
    ...
    return CreateTensorView(input, newOriginalShape, storageShape, newViewStride, Format::FORMAT_NCHW,
                            Format::FORMAT_NCHW, newStorageFormat, executor);
}
} // namespace
                                   ...
                                   aclOpExecutor* executor, bool c04Flag = false, bool transDataFlag = true)
{
    ...
    OP_CHECK(inputTensor != nullptr,
             OP_LOGE(ACLNN_ERR_INNER_NULLPTR,
                     "The input preprocess failed, %s with TransData"
                     " return nullptr.",
                     tensorName.c_str()),
             return ACLNN_ERR_INNER_NULLPTR);
    return ACLNN_SUCCESS;
                                     ...
                                     const string& tensorName, const int64_t groups, aclOpExecutor* executor)
{
    ...
    if (needSpecialCast) {
        if (outputTensor->GetDataType() == op::DataType::DT_FLOAT) {
            ...
            OP_CHECK(l0ResultTensor != nullptr,
                     OP_LOGE(ACLNN_ERR_INNER_NULLPTR,
                             "The output postprocess fail, l0ResultTensor with cast return nullptr."),
                     return ACLNN_ERR_INNER_NULLPTR);
            const aclTensor* transTensor = nullptr;
            transTensor = l0op::TransData(l0ResultTensor, GetPrimaryFormat(outputTensor->GetOriginalFormat()), groups,
                                          executor);
            OP_CHECK(transTensor != nullptr,
                     OP_LOGE(ACLNN_ERR_INNER_NULLPTR,
                             "The output postprocess failed, %s with TransData return nullptr.", tensorName.c_str()),
                     return ACLNN_ERR_INNER_NULLPTR);
            ...
            OP_CHECK(l0ResultTensor != nullptr,
                     OP_LOGE(ACLNN_ERR_INNER_NULLPTR,
                             "The output postprocess fail, l0ResultTensor with cast return nullptr."),
                     return ACLNN_ERR_INNER_NULLPTR);
            outputTensor = l0op::TransData(l0ResultTensor, GetPrimaryFormat(outputTensor->GetOriginalFormat()), groups,
                                           executor);
            OP_CHECK(outputTensor != nullptr,
                     OP_LOGE(ACLNN_ERR_INNER_NULLPTR,
                             "The output postprocess failed, %s with transdata return nullptr.", tensorName.c_str()),
...
namespace {
static aclIntArray* View1dAs2dWithGroups(const int64_t& groups, const aclIntArray* intArray, const int64_t& expendValue,
                                         aclOpExecutor* executor, const string& tensorName)
{
...
static const aclTensor* View3dWithGroups(const int64_t& groups, const aclTensor* input, aclOpExecutor* executor,
                                         const string& tensorName)
{
    int64_t squeezeDim;
    if (groups == 1) {
...
static const aclTensor* View4dWithGroups(const int64_t& groups, const aclTensor* input, aclOpExecutor* executor,
                                         const string& tensorName)
{
    ...
    OP_CHECK(unsqueezedInput != nullptr,
    ...
    if ((*params.outputMask)[2]) {
        ...
        if (Ops::NN::AclnnUtil::IsRegbase(curArch)) {
            ...
            OP_CHECK(biasSupport,
                     OP_LOGE_FOR_INVALID_DTYPES_WITH_REASON(
                         ACLNN_CONVOLUTION_BACKWARD_NAME, "gradBias",
        ...
        const int64_t reshapeList[] = {inputTensor.gradOutput->GetViewShape().GetDim(0), reshapeListVal0,
                                       reshapeListVal1};
        ...
        OP_CHECK(gradReshape != nullptr, OP_LOGE(ACLNN_ERR_INNER_NULLPTR, "The Reshape with gradOutput failed."),
                 return ACLNN_ERR_INNER_NULLPTR);
        auto gradReshapeND = l0op::ReFormat(gradReshape, op::Format::FORMAT_ND);
        OP_CHECK(gradReshapeND != nullptr, OP_LOGE(ACLNN_ERR_INNER_NULLPTR, "The ReFormat with gradOutput failed."),
                 return ACLNN_ERR_INNER_NULLPTR);
        ...
        OP_CHECK(gradBiasResult != nullptr, OP_LOGE(ACLNN_ERR_INNER_NULLPTR, "The ReduceSumOp with gradOutput failed."),
                 return ACLNN_ERR_INNER_NULLPTR);
        OP_LOGD("gradBiasResult: %s", gradBiasResult->ToString().GetString());
        CHECK_RET(OutputPostProcess(outputTensor.gradBias, gradBiasResult, "gradBias", params.groups, executor) ==
                      ACLNN_SUCCESS,
                                                    ...
                                                    vector<bool>& conv3DBp2MatmulMask)
{
    ...
    const aclTensor* gradWeight1x1 = l0op::Conv2DBackpropFilterFp162Fp32(
        inputTensorForDw1x1.input, inputTensorForDw1x1.weight, inputTensorForDw1x1.gradOutput, paramsForDw1x1.stride,
        paramsForDw1x1.padding, paramsForDw1x1.dilation, paramsForDw1x1.groups, executor);
                                           ...
                                           aclOpExecutor* executor)
{
    ...
    OP_CHECK(!(Ops::NN::AclnnUtil::IsRegbase(curArch)),
             OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(
                 ACLNN_CONVOLUTION_BACKWARD_NAME, "npuarch", std::to_string(static_cast<uint32_t>(curArch)).c_str(),
    ...
    if ((*params.outputMask)[1] && !conv2DBp2MatmulMask[1]) {
        ...
        if (deterministicValue && curArch == NpuArch::DAV_2201 &&
            ...
            FVector<int64_t> newPadding = {
                0, 0, (*params.padding)[0], (*params.padding)[0], (*params.padding)[1], (*params.padding)[1]};
            if (params.padding->Size() == CONV2DINPUTDIM) {
                newPadding = {
                    0, 0, (*params.padding)[0], (*params.padding)[1], (*params.padding)[2], (*params.padding)[3]};
            }
            auto stride3d = executor->AllocIntArray(newStride.data(), newStride.size());
            OP_CHECK(stride3d != nullptr, OP_LOGD("Failed to allocate memory for stride3d"),
                     return ACLNN_ERR_INNER_NULLPTR);
            auto dilation3d = executor->AllocIntArray(newDilation.data(), newDilation.size());
            OP_CHECK(dilation3d != nullptr, OP_LOGD("Failed to allocate memory for dilation3d"),
                     return ACLNN_ERR_INNER_NULLPTR);
            auto padding3d = executor->AllocIntArray(newPadding.data(), newPadding.size());
            OP_CHECK(padding3d != nullptr, OP_LOGD("Failed to allocate memory for padding3d"),
                     return ACLNN_ERR_INNER_NULLPTR);

            auto newInput = View5hdas6hd(inputTensor.input, executor);
            OP_CHECK(newInput != nullptr, OP_LOGD("Failed to allocate memory for newInput"),
                     return ACLNN_ERR_INNER_NULLPTR);
            auto newGradOutput = View5hdas6hd(inputTensor.gradOutput, executor);
            OP_CHECK(newGradOutput != nullptr, OP_LOGD("Failed to allocate memory for newGradOutput"),
                     return ACLNN_ERR_INNER_NULLPTR);
            auto newWeight = ViewFZasFZ3D(inputTensor.weight, executor);
            OP_CHECK(newWeight != nullptr, OP_LOGD("Failed to allocate memory for newWeight"),
                     return ACLNN_ERR_INNER_NULLPTR);
...
static aclnnStatus CalculateConv1DBackwardByMatmulImpl(ConvolutionBackwardInputTensor& inputTensor,
                                                       ConvolutionBackwardResult& outputTensor,
                                                       ConvolutionBackwardParams& params, aclOpExecutor* executor)
{
    ...
    if ((*params.outputMask)[0]) {
        ...
        auto mmGradInput = ExecBatchMatmulOp(gradOutputND, weightND, gradInputND, false, false, params.cubeMathType,
                                             executor);
        ...
        OP_CHECK(result != nullptr,
                 OP_LOGE(ACLNN_ERR_INNER_NULLPTR,
                         "The output view process failed, gradInput with ViewCopy in matmul return nullptr"),
                 return ACLNN_ERR_INNER_NULLPTR);
    }
    ...
    if ((*params.outputMask)[1]) {
        ...
        auto gradWeightND = ViewWithShapeAndReformatND(outputTensor.gradWeight, {outChannels, inChannels * l},
                                                       executor);
        CHECK_RET(gradWeightND != nullptr, ACLNN_ERR_INNER_NULLPTR);
        auto mmGradWeight = ExecBatchMatmulOp(gradOutputND, inputND, gradWeightND, true, false, params.cubeMathType,
                                              executor);
        ...
        OP_CHECK(result != nullptr,
                 OP_LOGE(ACLNN_ERR_INNER_NULLPTR,
                         "The output view process failed, gradWeight with ViewCopy in matmul return nullptr."),
                 return ACLNN_ERR_INNER_NULLPTR);
    }
    return ACLNN_SUCCESS;
}

static aclnnStatus PreConv1DBackwardTo2D(ConvolutionBackwardInputTensor& inputTensor,
                                         ConvolutionBackwardResult& outputTensor, ConvolutionBackwardParams& params,
                                         aclOpExecutor* executor)
{
...
static aclnnStatus PostConv1DBackwardTo2D(ConvolutionBackwardResult& outputTensor, ConvolutionBackwardParams& params,
                                          aclOpExecutor* executor)
{
    int64_t exchangeDim = params.groups; // = 1 ==> w
                                          ...
                                          ConvolutionBackwardParams& params)
{
    ...
    if (!Ops::NN::AclnnUtil::IsRegbase() && isStrideEqKernel &&
...
static aclnnStatus GenDxInOutByConvBp2MmMode(BatchMatmulInput& batchMmInput,
                                             ...
                                             ConvolutionBackwardResult& outputTensor, aclOpExecutor* executor,
                                             Conv3DBp2MmMode conv2MmMode)
{
    ...
    auto gradOutputND = ViewWithShapeAndReformatND(
        gradOutput, {gradShapeVec[bDimBMNIdx], gradShapeVec[mDimBMNIdx], gradShapeVec[nDimBMNIdx]}, executor);
    CHECK_RET(gradOutputND != nullptr, ACLNN_ERR_INNER_NULLPTR);
    ...
    if (conv2MmMode == Conv3DBp2MmMode::CONV3D_BP_MM_STRIDE_EQ_KERNEL) {
        ...
        outShapeVec = {gradInput->GetViewShape()[nDimNCDHWIdx], gradInput->GetViewShape()[cDimNCDHWIdx],
                       CalcCountByAxisVec(gradInput->GetViewShape(), dhwIdxUnionVec)};
    }
    auto mmDxOutputND = ViewWithShapeAndReformatND(
        gradInput, {outShapeVec[bDimBMNIdx], outShapeVec[mDimBMNIdx], outShapeVec[nDimBMNIdx]}, executor);
                                                     ...
                                                     Conv3DBp2MmMode conv2MmMode)
{
    ...
    if (conv2MmMode == Conv3DBp2MmMode::CONV3D_BP_MM_STRIDE_EQ_KERNEL) {
        ...
        op::Shape mmOutputExtendShape = op::Shape({
            gradOutputShape[0],
            gradOutputShape[dDimNCDHWIdx],
            gradOutputShape[hDimNCDHWIdx],
            gradOutputShape[wDimNCDHWIdx],
            weightShape[ciDimCoCiDHWIdx],
            weightShape[dDimNCDHWIdx],
            weightShape[hDimNCDHWIdx],
            weightShape[wDimNCDHWIdx],
        });
...
// FM=KERNEL
static aclnnStatus GenConvMmDwInputByMode(BatchMatmulInput& batchMmInput, ConvolutionBackwardInputTensor& inputTensor,
                                          aclOpExecutor* executor)
{
    auto gradOutput = inputTensor.gradOutput; // dy
    auto input = inputTensor.input;           // x
    op::Shape gradOutputShape2d = op::Shape(
        {1, gradOutput->GetViewShape()[0], CalcCountByAxisVec(gradOutput->GetViewShape(), {1, 2, 3, 4})});
    ...
    op::Shape inputShape2d = op::Shape(
        {1, input->GetViewShape()[0], CalcCountByAxisVec(input->GetViewShape(), {1, 2, 3, 4})});
...
static aclnnStatus GenConvMmDwInputBy1x1Mode(BatchMatmulInput& batchMmInput,
                                             ConvolutionBackwardInputTensor& inputTensor, aclOpExecutor* executor)
{
    // 1x1x1 kernel + batch=1 场景下，dw 可转为 2D matmul：
    // gradOutput [N=1, Co, D, H, W] -> [1, Co, D*H*W]，作为左矩阵(不转置)
    // input       [N=1, Cin, D, H, W] -> [1, Cin, D*H*W]，作为右矩阵(转置)
    // matmul 输出 [1, Co, Cin]，对应 gradWeight [Co, Cin, 1, 1, 1]
    const vector<int64_t> dhwIdxUnionVec{dDimNCDHWIdx, hDimNCDHWIdx, wDimNCDHWIdx};
    auto gradOutput = inputTensor.gradOutput;
    auto input = inputTensor.input;
    op::Shape gradOutputShape2d = op::Shape(
        {1, gradOutput->GetViewShape()[cDimNCDHWIdx], CalcCountByAxisVec(gradOutput->GetViewShape(), dhwIdxUnionVec)});
    auto gradOutput2d = ViewWithShape(gradOutput, gradOutputShape2d, executor);
    CHECK_RET(gradOutput2d != nullptr, ACLNN_ERR_INNER_NULLPTR);
    auto gradOutputND = l0op::ReFormat(gradOutput2d, op::Format::FORMAT_ND);

    op::Shape inputShape2d = op::Shape(
        {1, input->GetViewShape()[cDimNCDHWIdx], CalcCountByAxisVec(input->GetViewShape(), dhwIdxUnionVec)});
    auto input2d = ViewWithShape(input, inputShape2d, executor);
    CHECK_RET(input2d != nullptr, ACLNN_ERR_INNER_NULLPTR);
    auto inputND = l0op::ReFormat(input2d, op::Format::FORMAT_ND);

    batchMmInput.leftData = gradOutputND;
    batchMmInput.isLeftTranspose = false;
    batchMmInput.rightData = inputND;
    batchMmInput.isRightTranspose = true;
    return ACLNN_SUCCESS;
}

static aclnnStatus GenConvMmDwOutputByMode(aclTensor*& mmDwOutput, ConvolutionBackwardResult& outputTensor,
                                           aclOpExecutor* executor)
{
    auto gradWeight = outputTensor.gradWeight;
    op::Shape mmDwOutShape2d = op::Shape({1,
    ...
    mmDwOutput = executor->CreateView(mmDwOutputND, mmDwOutputND->GetViewShape(), mmDwOutputND->GetViewOffset());
...
static aclnnStatus CalculateConv3DBackwardDwByMmMode(ConvolutionBackwardInputTensor& inputTensor,
                                                     ConvolutionBackwardResult& outputTensor,
                                                     ConvolutionBackwardParams& params, aclOpExecutor* executor,
                                                     bool w1B1FmNDwTransToMmFlag = false)
{
    BatchMatmulInput batchMmInput;
    aclnnStatus status;
    if (w1B1FmNDwTransToMmFlag) {
        OP_LOGD("Enter 1x1 dw matmul optimization path");
        status = GenConvMmDwInputBy1x1Mode(batchMmInput, inputTensor, executor);
    } else {
        status = GenConvMmDwInputByMode(batchMmInput, inputTensor, executor);
    }
    ...
    status = GenConvMmDwOutputByMode(mmDwOutput, outputTensor, executor);
    if (status != ACLNN_SUCCESS) {
        OP_LOGD("GenConvMmDwOutputByMode False");
        return status;
    }
    auto gradWeightNND = ExecBatchMatmulOp(batchMmInput.leftData, batchMmInput.rightData, mmDwOutput,
                                           batchMmInput.isLeftTranspose, batchMmInput.isRightTranspose,
                                           params.cubeMathType, executor);
...
static bool IsGreaterL2Cache(const ConvolutionBackwardInputTensor& inputTensor, const ConvolutionBackwardParams& params)
{
    op::Shape inputShape = params.transposed ? inputTensor.gradOutput->GetViewShape() :
                                               inputTensor.input->GetViewShape();
    op::Shape weightShape = inputTensor.weight->GetViewShape();
                                 ...
                                 const ConvolutionBackwardParams& params)
{
    ...
    OP_LOGD("Original ConvBackpropInput can convert to Matmul, "
            "from [w.shape(Cout, Cin, 1, 1, 1), Dy.shape(1, Cout, Dout=Din, Hout=Hin, Wout=Win), Dx.shape(1, Cin, Din, "
            "Hin, Win)] "
            "to [a.shape(Cout, Cin), b.shape(Cout, Din*Hin*Win), c.shape(Cin, Din*Win*Hin)].");
    return true;
}

static bool IsGreaterL2CacheForDw1x1(const ConvolutionBackwardInputTensor& inputTensor)
{
    op::Shape weightShape = inputTensor.weight->GetViewShape();
    op::Shape inputShape = inputTensor.input->GetViewShape();
    int64_t cOutDim = weightShape.GetDim(NCDHW_N_DIM);
    int64_t cInDim = weightShape.GetDim(NCDHW_C_DIM);
    int64_t dInDim = inputShape.GetDim(NCDHW_D_DIM);
    int64_t hInDim = inputShape.GetDim(NCDHW_H_DIM);
    int64_t wInDim = inputShape.GetDim(NCDHW_W_DIM);
    // dw 1x1 转Matmul: gradOutput[Co, D*H*W] x input[Cin, D*H*W]^T -> gradWeight[Co, Cin]
    int64_t m = cOutDim;
    int64_t k = dInDim * hInDim * wInDim;
    int64_t n = cInDim;
    int64_t typeSize = ge::GetSizeByDataType(inputTensor.input->GetDataType());
    // 类型错误
    if (typeSize == -1) {
    ...
    const int64_t l2CacheSize = 128 * 1024 * 1024;
    // 数据量超过l2cache(128MB)时不走matmul路径
    if ((m * k + k * n + n * m) * typeSize > l2CacheSize) {
        OP_LOGD("dw 1x1 shape exceeds L2Cache, can not convert to Matmul, m = %ld, k = %ld, n = %ld", m, k, n);
        return true;
    }
    return false;
}

static bool Is1x1DwTransToMm(const ConvolutionBackwardInputTensor& inputTensor, const ConvolutionBackwardParams& params)
{
    // only support  ASCEND950
    if (!(Ops::NN::AclnnUtil::IsRegbase())) {
        return false;
    ...
    // batch=1
    if (inputTensor.input->GetViewShape()[nDimNCDHWIdx] != 1) {
        return false;
    }
    // groups=1
    if (params.groups != 1) {
    ...
    // 是否是超大shape,超过l2cache(128MB)进行拦截
    if (IsGreaterL2CacheForDw1x1(inputTensor)) {
        return false;
    }
    return true;
}

static aclnnStatus CalculateW1B1FmNDxByMm(ConvolutionBackwardInputTensor& inputTensor,
                                          ConvolutionBackwardResult& outputTensor, ConvolutionBackwardParams& params,
                                          aclOpExecutor* executor)
{
                                                       ...
                                                       vector<bool>& conv3DBp2MatmulMask)
{
    ...
    bool w1B1FmNDwTransToMmFlag = Is1x1DwTransToMm(inputTensor, params);
    auto conv2MmMode = GetConv3DBp2MmMode(inputTensor, params);
    if (conv2MmMode == Conv3DBp2MmMode::CONV3D_BP_NO_MM && !w1B1FmNDxTransToMmFlag && !w1B1FmNDwTransToMmFlag) {
        return ACLNN_SUCCESS;
    }
    if ((*params.outputMask)[0]) {
        if (w1B1FmNDxTransToMmFlag) {
            ...
            if (conv2MmMode != Conv3DBp2MmMode::CONV3D_BP_NO_MM) {
                ...
                auto gradInputND = ExecBatchMatmulOp(batchMmInput.leftData, batchMmInput.rightData,
                ...
                status = OutputPostProcess(outputTensor.gradInput, gradInputNCDHW, "gradInput", params.groups,
                                           executor);
                CHECK_RET(status == ACLNN_SUCCESS, status);
                conv3DBp2MatmulMask[0] = true;
            }
    ...
    // dw: FM=KERNEL场景 或 1x1x1 kernel + batch=1 + stride=1 + pad=0 + dilation=1 + groups=1场景
    if ((*params.outputMask)[1] &&
        (conv2MmMode == Conv3DBp2MmMode::CONV3D_BP_MM_FEATURE_MAP_EQ_KERNEL || w1B1FmNDwTransToMmFlag)) {
        // FM=KERNEL与1x1重叠时优先走FM=KERNEL路径
        if (conv2MmMode == Conv3DBp2MmMode::CONV3D_BP_MM_FEATURE_MAP_EQ_KERNEL) {
            w1B1FmNDwTransToMmFlag = false;
        }
        auto status = CalculateConv3DBackwardDwByMmMode(inputTensor, outputTensor, params, executor,
                                                        w1B1FmNDwTransToMmFlag);
        CHECK_RET(status == ACLNN_SUCCESS, status);
        conv3DBp2MatmulMask[1] = true;
    }
    return ACLNN_SUCCESS;
...
static const aclTensor* GetGradInputNDC1HWC0(const ConvolutionBackwardInputTensor& inputTensor,
                                             const ConvolutionBackwardParams& params, aclOpExecutor* executor,
                                             bool useHf32, AdaptParam* adptParams)
{
    const aclTensor* gradInputNDC1HWC0 = nullptr;
    if (useHf32) {
        gradInputNDC1HWC0 = l0op::Conv3DBackpropInputHf32(inputTensor.input, inputTensor.weight, inputTensor.gradOutput,
                                                          params.stride, params.padding, params.dilation, params.groups,
                                                          executor, adptParams);
    } else if (inputTensor.weight->GetDataType() == DataType::DT_FLOAT) {
        gradInputNDC1HWC0 = l0op::Conv3DBackpropInputFp322Fp32(inputTensor.input, inputTensor.weight,
                                                               inputTensor.gradOutput, params.stride, params.padding,
                                                               params.dilation, params.groups, executor, adptParams);
    } else if (inputTensor.input->GetDataType() == DataType::DT_BF16) {
...
static const aclTensor* GetGradWeightFZ3D(const ConvolutionBackwardInputTensor& inputTensor,
...
bool IsTransTo1x1Dw(const ConvolutionBackwardInputTensor& inputTensor, const ConvolutionBackwardParams& params)
{
    ...
    op::Shape gradOutShape = params.transposed ? inputTensor.input->GetViewShape() :
                                                 inputTensor.gradOutput->GetViewShape();
    bool isDEqual = gradOutShape.GetDim(NCDHW_D_DIM) * weightShape.GetDim(NCDHW_D_DIM) ==
                    inputShape.GetDim(NCDHW_D_DIM);
    bool isHEqual = gradOutShape.GetDim(NCDHW_H_DIM) * weightShape.GetDim(NCDHW_H_DIM) ==
                    inputShape.GetDim(NCDHW_H_DIM);
    bool isWEqual = gradOutShape.GetDim(NCDHW_W_DIM) * weightShape.GetDim(NCDHW_W_DIM) ==
                    inputShape.GetDim(NCDHW_W_DIM);
    if (!isDEqual || !isHEqual || !isWEqual) {
        return false;
    }
    // 性能判定条件：需要 hk >= 8 and wk >= 8
    if ((*params.stride)[CONV3D_ATTR_H_IDX] < 8 ||
        (*params.stride)[CONV3D_ATTR_W_IDX] < 8) { // 8: 经验值，stride越大Dw搬运膨胀越大，越容易回本
        return false;
    }
                                                    ...
                                                    bool useHf32)
{
    ...
    tmpShape = op::Shape({cOutDim, cInDim * dKernelDim * hKernelDim * wKernelDim, 1, 1, 1});
    auto wTensorForDw1x1 = ViewWithShape(wTensor, tmpShape, executor);
                                                    ...
                                                    aclOpExecutor* executor)
{
    ...
    if (Ops::NN::AclnnUtil::IsRegbase() &&
        CheckN2HEnable(tempTensor.weight, tempTensor.input, stride5, dilation5, pad6, params.groups)) {
                                           ...
                                           aclOpExecutor* executor)
{
    ...
    l0op::ConvBackpropParams conv3DBackpropPrarams = {inputTensor.input, inputTensor.weight, inputTensor.gradOutput,
                                                      params.stride,     params.padding,     params.dilation,
                                                      params.groups};
    auto mmStatus = CalculateConv3DBackwardByMatmulImpl(inputTensor, outputTensor, params, executor,
                                                        conv3DBp2MatmulMask);
    CHECK_RET(mmStatus == ACLNN_SUCCESS, mmStatus);
    ...
    if (!(Ops::NN::AclnnUtil::IsRegbase(curArch))) {
        ...
        inputTransDataFlag = !(
            (*params.outputMask)[1] && l0op::IsConv3DBackpropFilterV2(conv3DBackpropPrarams) &&
            l0op::IsInputTransdataWhiteListCase(conv3DBackpropPrarams)); // dw、v2、白名单同时满足，输入x融合transdata;
    ...
    if ((*params.outputMask)[1] && !conv3DBp2MatmulMask[1]) {
        ...
        if (Ops::NN::AclnnUtil::IsRegbase(curArch)) {
            ...
            OP_CHECK(gradWeightND != nullptr,
                     ...
                     return ACLNN_ERR_INNER_NULLPTR);
            CHECK_RET(OutputPostProcessWithoutTransdata(outputTensor.gradWeight, gradWeightND, "gradWeight",
            ...
            OP_CHECK(gradWeightFZ3D != nullptr,
                     OP_LOGE(ACLNN_ERR_INNER_NULLPTR,
                             "The calculation with empty tensor failed, Conv2dBackpropInput return nullptr."),
                     return ACLNN_ERR_INNER_NULLPTR);
            CHECK_RET(OutputPostProcess(outputTensor.gradWeight, gradWeightFZ3D, "gradWeight", params.groups,
                                        executor) == ACLNN_SUCCESS,
                      ACLNN_ERR_INNER_NULLPTR);
        }
    }
                                                    ...
                                                    ConvolutionBackwardParams& params, aclOpExecutor* executor)
{
    ...
    if ((*params.outputMask)[1]) {
        ...
        OP_CHECK(gradWeightFZ3D != nullptr,
                 ...
                 return ACLNN_ERR_INNER_NULLPTR);
        if (Ops::NN::AclnnUtil::IsRegbase()) {
            CHECK_RET(OutputPostProcessWithoutTransdata(outputTensor.gradWeight, gradWeightFZ3D, "gradWeight",
                                                        executor) == ACLNN_SUCCESS,
                      ACLNN_ERR_INNER_NULLPTR);
        } else {
            CHECK_RET(OutputPostProcess(outputTensor.gradWeight, gradWeightFZ3D, "gradWeight", params.groups,
                                          ...
                                          const ConvolutionBackwardParams& params)
{
    ...
    if (Ops::NN::AclnnUtil::IsRegbase()) {
        if (cubeMathType == ALLOW_FP32_DOWN_PRECISION &&
            ...
            OP_LOGW("When promoteDtype(%s) is hifloat8/float8_e4m3fn, the cubeMathType can not be "
...
static bool isConv2dTo3d(const ConvolutionBackwardInputTensor& inputTensor, const ConvolutionBackwardParams& params)
{
    auto curArch = GetCurrentPlatformInfo().GetCurNpuArch();
    auto inputDim = inputTensor.input->GetViewShape().GetDimNum();
    if (inputDim != CONV2DINPUTDIM) {
                                                ...
                                                ConvolutionBackwardParams& params, aclOpExecutor* executor)
{
    ...
    OP_CHECK(inputTensor.input != nullptr, OP_LOGE(ACLNN_ERR_INNER_NULLPTR, "Swap D and H for input failed."),
             return ACLNN_ERR_INNER_NULLPTR);

    inputTensor.gradOutput = SwapDHDimensions(inputTensor.gradOutput, executor);
    OP_CHECK(inputTensor.gradOutput != nullptr, OP_LOGE(ACLNN_ERR_INNER_NULLPTR, "Swap D and H for gradOutput failed."),
             return ACLNN_ERR_INNER_NULLPTR);

    inputTensor.weight = SwapDHDimensions(inputTensor.weight, executor);
    OP_CHECK(inputTensor.weight != nullptr, OP_LOGE(ACLNN_ERR_INNER_NULLPTR, "Swap D and H for weight failed."),
             return ACLNN_ERR_INNER_NULLPTR);
    ...
    if (params.stride->Size() == CONV3D_ATTR_DIM) {
        ...
        OP_CHECK(params.stride != nullptr, OP_LOGE(ACLNN_ERR_INNER_NULLPTR, "AllocIntArray for stride failed."),
                 return ACLNN_ERR_INNER_NULLPTR);
    }

    if (params.dilation->Size() == CONV3D_ATTR_DIM) {
        params.dilation = SwapDHInArray3(params.dilation, executor);
        OP_CHECK(params.dilation != nullptr, OP_LOGE(ACLNN_ERR_INNER_NULLPTR, "AllocIntArray for dilation failed."),
                 return ACLNN_ERR_INNER_NULLPTR);
    }

    if (params.padding->Size() == CONV3D_PAD_DIM) {
        params.padding = SwapDHInPaddingArray6(params.padding, executor);
        OP_CHECK(params.padding != nullptr, OP_LOGE(ACLNN_ERR_INNER_NULLPTR, "AllocIntArray for padding failed."),
                 return ACLNN_ERR_INNER_NULLPTR);
                                     ...
                                     aclOpExecutor* executor)
{
    ...
    OP_CHECK(curArch == NpuArch::DAV_2201 || Ops::NN::AclnnUtil::IsRegbase(curArch),
             OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(ACLNN_CONVOLUTION_BACKWARD_NAME, "npuArch",
                                                   ...
                                                   "the value of npuarch must not be 2201 or 3510"),
                                     ...
                                     aclOpExecutor* executor)
{
    ...
    if (isConv2DTo3D) {
        ...
        OP_CHECK(inputTensor.gradOutput != nullptr, OP_LOGD("gradOutput squeeze failed."),
                 return ACLNN_ERR_INNER_NULLPTR);
        ...
        if ((*params.outputMask)[0]) { // 计算dx时
            ...
            OP_CHECK(outputTensor.gradInput != nullptr, OP_LOGD("gradInput squeeze failed."),
                     return ACLNN_ERR_INNER_NULLPTR);
        }
        if ((*params.outputMask)[1]) { // 计算dw时
            outputTensor.gradWeight = View4Das5D(outputTensor.gradWeight, executor);
            OP_CHECK(outputTensor.gradWeight != nullptr, OP_LOGD("gradWeight squeeze failed."),
                     return ACLNN_ERR_INNER_NULLPTR);
        }
        ...
        if ((*params.outputMask)[0]) {
            ...
            OP_CHECK(outputTensor.gradInput != nullptr, OP_LOGD("gradInput unsqueeze failed."),
                     return ACLNN_ERR_INNER_NULLPTR);
        }
        if ((*params.outputMask)[1]) {
            outputTensor.gradWeight = View5Das4D(outputTensor.gradWeight, executor);
            OP_CHECK(outputTensor.gradWeight != nullptr, OP_LOGD("gradWeight unsqueeze failed."),
                     return ACLNN_ERR_INNER_NULLPTR);
        }
                                                ...
                                                ConvolutionBackwardParams& params, aclOpExecutor* executor)
{
    ...
    if (inputDim == CONV1DINPUTDIM) {
        ...
        OP_LOGE_FOR_INVALID_SHAPEDIM(ACLNN_CONVOLUTION_BACKWARD_NAME, "input", std::to_string(inputDim).c_str(),
                                     "3 or 4 or 5");
        return ACLNN_ERR_PARAM_INVALID;
    }
                                                            ...
                                                            aclOpExecutor* executor)
{
    if (gradInput->Size() != 0) {
        ...
        const aclTensor* inputDims = executor->ConvertToTensor(inputFillShape.data(), inputFillShape.size(),
    ...
    if (gradWeight->Size() != 0) {
        ...
        const aclTensor* weightDims = executor->ConvertToTensor(weightFillShape.data(), weightFillShape.size(),
                                                                op::DataType::DT_INT64);
...
static aclnnStatus CalculateConvolutionTbcBackward(ConvolutionBackwardInputTensor& inputTensor,
                                                   ...
                                                   ConvolutionBackwardParams& params, aclOpExecutor* executor)
{
    aclTensor* gradInputNcl = executor->AllocTensor(inputTensor.input->GetViewShape(),
                                                    inputTensor.input->GetDataType());
    ...
    if (inputDim == 3) {
        ...
        OP_LOGE_FOR_INVALID_SHAPEDIM(ACLNN_CONV_TBC_BACKWARD_NAME, "input", std::to_string(inputDim).c_str(), "3");
        return ACLNN_ERR_INNER_NULLPTR;
    }
    // recover NCL to TBC
...
static const aclTensor* ReshapeTensor(const aclTensor* input, aclOpExecutor* executor, const ReshapeTargetConf& conf)
{
    ...
    OP_CHECK(reshapeInput != nullptr,
             OP_LOGE(ACLNN_ERR_INNER_NULLPTR, "The Reshape with %s failed.", conf.tensorName.c_str()), return nullptr);
    auto finalInput = l0op::ReFormat(reshapeInput, conf.targetFormat);
    OP_CHECK(finalInput != nullptr,
             OP_LOGE(ACLNN_ERR_INNER_NULLPTR, "The ReFormat with %s failed.", conf.tensorName.c_str()), return nullptr);
    return finalInput;
}
...
static const aclTensor* ReshapeTBCTo3DFormat(const aclTensor* input, aclOpExecutor* executor, const string& tensorName)
{
    ...
    if (tensorName == "weight" || tensorName == "gradWeight") {
        conf.dims = {1, input->GetViewShape().GetDim(0), 1, input->GetViewShape().GetDim(1),
                     -1}; // [1, K, 1, Cin, Cout]
                                                       ...
                                                       ConvolutionBackwardParams& params, aclOpExecutor* executor)
{
    ...
    aclTensor* gradWeight = executor->AllocTensor(inputTensor.weight->GetViewShape(),
                                                  inputTensor.weight->GetDataType());
    aclTensor* gradBias = executor->AllocTensor(finalTensor.gradBias->GetViewShape(),
                                                finalTensor.gradBias->GetDataType());
    ...
    if ((*params.outputMask)[1]) {
        resultTensor.gradWeight = Reshape3DFormatToTBC(
            resultTensor.gradWeight, finalTensor.gradWeight->GetStorageFormat(), executor, "gradWeight");
        CHECK_RET(resultTensor.gradWeight != nullptr, ACLNN_ERR_INNER_NULLPTR);
    }
    ...
    OP_CHECK(gradInputResult != nullptr,
             OP_LOGE(ACLNN_ERR_INNER_NULLPTR,
                     "The aclnnConvTbcBackward output viewprocess failed, gradInput with ViewCopy return nullptr."),
             return ACLNN_ERR_INNER_NULLPTR);
    ...
    OP_CHECK(gradWeightResult != nullptr,
             OP_LOGE(ACLNN_ERR_INNER_NULLPTR,
                     "The aclnnConvTbcBackward output viewprocess failed, gradWeight with ViewCopy return nullptr."),
             return ACLNN_ERR_INNER_NULLPTR);
    auto gradBiasResult = l0op::ViewCopy(resultTensor.gradBias, finalTensor.gradBias, executor);
    OP_CHECK(gradBiasResult != nullptr,
             OP_LOGE(ACLNN_ERR_INNER_NULLPTR,
                     "The aclnnConvTbcBackward output viewprocess failed, gradBias with ViewCopy return nullptr."),
             return ACLNN_ERR_INNER_NULLPTR);
    return ACLNN_SUCCESS;
}
} // namespace
...
static aclnnStatus ResultViewProcess(Ops::NN::Conv::ConvolutionBackwardResult& ResultTensor,
                                     Ops::NN::Conv::ConvolutionBackwardOutput& outputTensor, aclOpExecutor* executor)
{
    // APIViewCopy
    // Index  0ViewCopy gradInput
    auto resultinput = l0op::ViewCopy(ResultTensor.gradInput, outputTensor.gradInput, executor);
    OP_CHECK(resultinput != nullptr,
             OP_LOGE(ACLNN_ERR_INNER_NULLPTR, "The output viewprocess failed, gradInput with ViewCopy return nullptr."),
             return ACLNN_ERR_INNER_NULLPTR);
...
static aclnnStatus TransProcess(const aclTensor*& inputTensor, const string& tensorName, aclOpExecutor* executor,
                                bool isInput)
{
...
static aclnnStatus CalculateDeformableOffsetsGrad(Ops::NN::Conv::DeformableConv2dBackwardInputTensor& inputTensor,
                                                  ConvolutionBackwardResult& bp_resultTensor, const aclTensor* offset,
                                                  Ops::NN::Conv::DeformableConv2dBackwardParams& params,
                                                  const aclTensor** gradInput, const aclTensor** gradOffset,
                                                  aclOpExecutor* executor)
{
    ...
    CHECK_RET(TransProcess(bp_resultTensor.gradInput, "gradInput", executor, true) == ACLNN_SUCCESS,
              ACLNN_ERR_INNER_NULLPTR);
...
static aclnnStatus CalculateDeformableConv2dBackward(Ops::NN::Conv::DeformableConv2dBackwardInputTensor& inputTensor,
                                                     Ops::NN::Conv::ConvolutionBackwardOutput& outputTensor,
                                                     const aclTensor* offset,
                                                     Ops::NN::Conv::DeformableConv2dBackwardParams& params,
                                                     aclOpExecutor* executor)
{
    Ops::NN::Conv::ConvolutionBackwardResult resultTensor = {outputTensor.gradInput, outputTensor.gradWeight,
                                                             outputTensor.gradOffset, outputTensor.gradBias};
    OP_LOGD("Enter CalculateConvolutionBackward for CalculateDeformableConv2dBackward.");
    auto bp_gradInput = executor->AllocTensor(inputTensor.offsetOut->GetViewShape(),
                                              inputTensor.offsetOut->GetDataType());
    ...
    CHECK_RET(CalculateDeformableOffsetsGrad(inputTensor, bp_resultTensor, offset, params, &gradInput, &gradOffset,
                                             executor) == ACLNN_SUCCESS,
              ACLNN_ERR_INNER_NULLPTR);
    ...
    CHECK_RET(
        OutputPostProcess(resultTensor.gradOffset, gradOffset, "gradOffset", params.groups, executor) == ACLNN_SUCCESS,
        ACLNN_ERR_INNER_NULLPTR);
    CHECK_RET(
        OutputPostProcess(resultTensor.gradInput, gradInput, "gradInput", params.groups, executor) == ACLNN_SUCCESS,
        ACLNN_ERR_INNER_NULLPTR);
    CHECK_RET(
        OutputPostProcess(resultTensor.gradWeight, gradWeight, "gradWeight", params.groups, executor) == ACLNN_SUCCESS,
        ACLNN_ERR_INNER_NULLPTR);
    CHECK_RET(OutputPostProcess(resultTensor.gradBias, gradBias, "gradBias", params.groups, executor) == ACLNN_SUCCESS,
              ACLNN_ERR_INNER_NULLPTR);
...
aclnnStatus aclnnDeformableConv2dBackwardGetWorkspaceSize(
    ...
    aclOpExecutor** executor)
{
    L2_DFX_PHASE_1(aclnnDeformableConv2dBackward,
                   DFX_IN(input, gradOutput, offsetOut, weight, offset, kernelSize, stride, padding, dilation, groups,
    ...
    Ops::NN::Conv::DeformableConv2dBackwardInputTensor inputTensor = {gradOutput, input, weight, offset, offsetOut};
    ...
    // 固定写法，参数检查
    NpuArch npuArch = GetCurrentPlatformInfo().GetCurNpuArch();
    Ops::NN::Conv::DeformableConv2dBackwardChecker deformableConv2dBackwardChecker(inputTensor, outputTensor, params,
                                                                                   npuArch);
...
aclnnStatus aclnnDeformableConv2dBackward(void* workspace, uint64_t workspaceSize, aclOpExecutor* executor,
                                          const aclrtStream stream)
{
