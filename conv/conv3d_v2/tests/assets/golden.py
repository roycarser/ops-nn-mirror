...
__golden__ = {
    "kernel": {"conv3dv2": "conv3d_v2_golden", "conv3d_v2": "conv3d_v2_golden"}
}
...
def to_NCDHW_from_FRACTAL_Z_3D(data, ori_shape, groups=1):
    ...
    mag_factor1 = _lcm(cout_ori, n0) // cout_ori
    mag_factor = min(_lcm(mag_factor0, mag_factor1), groups)

    cin_g = align(mag_factor * cin_ori, c0)
    cout_g = align(mag_factor * cout_ori, n0)
    real_g = ceil_div(groups, mag_factor)
    cin1_g = cin_g // c0
    cout1_g = cout_g // n0

    # FRACTAL_Z_3D shape: (real_g * d * cin1_g * h * w, cout1_g, n0, c0)
    data = data.reshape((real_g, d, cin1_g, h, w, cout1_g, n0, c0))
...
def conv3d_v2_golden(
    ...
    **kwargs,
    ...
    if is_950:
        ...
        x_np, filter_np = process_formats_a2_a3(
            x, filter, input_formats, input_ori_shapes, groups
        )
    ...
    is_dequant = x_dtype_str == "int8" and scale_np is not None
    bias_torch = (
        torch.from_numpy(bias_np) if bias_np is not None and not is_dequant else None
    )
    ...
    if scale_np is not None:
        if scale_np.ndim == 1:
            scale_tensor = torch.from_numpy(
                scale_np.reshape(1, scale_np.shape[0], 1, 1, 1)
            )
        elif scale_np.ndim == 5:
            scale_tensor = torch.from_numpy(scale_np)
        else:
            raise ValueError(
                f"scale must be 1D or 5D, got {scale_np.ndim}D with shape {scale_np.shape}"
            )
        out = torch.multiply(out, scale_tensor)
        if is_dequant and bias_np is not None:
            bias_tensor = torch.from_numpy(bias_np.reshape(1, -1, 1, 1, 1))
            out = torch.add(out, bias_tensor)
        out = out.numpy()
    else:
        out = out.numpy()
