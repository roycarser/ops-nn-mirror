...
/*!
 * \file test_geir_deformable_offsets_grad.cpp
 * \brief
 */
#include <iostream>
#include <fstream>
...
#define ADD_INPUT(inputIndex, inputName, inputDtype, inputShape)                                                     \
    ...
    TensorDesc placeholder##inputIndex##_desc =                                                                      \
        TensorDesc(ge::Shape(placeholder##inputIndex##_shape), ge::FORMAT_NHWC, inputDtype);                         \
    placeholder##inputIndex##_desc.SetPlacement(ge::kPlacementHost);                                                 \
    placeholder##inputIndex##_desc.SetFormat(ge::FORMAT_NHWC);                                                       \
    ...
    Graph& graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto add1 = op::DeformableOffsetsGrad("deformable_offsets_grad");
    ...
    ADD_INPUT(1, grad, inDtype, shapes[0]);
    ADD_INPUT(2, x, inDtype, shapes[1]);
    ADD_INPUT(3, offsets, inDtype, shapes[2]);
    ADD_OUTPUT(4, grad_x, inDtype, shapes[3]);
    ADD_OUTPUT(5, grad_offsets, inDtype, shapes[4]);

    add1.set_attr_strides(shapes[5]);
    add1.set_attr_pads(shapes[6]);
    add1.set_attr_ksize(shapes[7]);
    add1.set_attr_dilations(shapes[8]);
    ADD_INPUT_ATTR(data_format, "NHWC");
    std::int64_t deformable_groups_ = 8;
    add1.set_attr_deformable_groups(deformable_groups_);
    bool modulated_ = true;
    add1.set_attr_modulated(modulated_);
