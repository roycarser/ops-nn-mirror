...
/*!
 * \file conv_bp_input_sub_func_load_gm_to_l1.h
 * \brief
 */
                                     ...
                                     uint64_t out2B1SrcAddrOffset, const LocalTensor<typename Intf::SrcBT>& useB1Buf)
{
    ...
    dn2NzParams.srcDnMatrixStride = self->ctx.tiling_->cinG * self->ctx.tiling_->dkHkWk; // src B stride (unit: element)
    dn2NzParams.dstNzMatrixStride = self->ctx.tiling_->c0;                               // dst B stride (unit: element)

    dn2NzParams.dValue = curCinSize;                                           // D: cin
    dn2NzParams.srcDValue = self->ctx.tiling_->dkHkWk;                         // src D stride (unit: element)
                                     ...
                                     uint64_t out2B1SrcAddrOffset, const LocalTensor<typename Intf::SrcBT>& useB1Buf)
{
    ...
    nd2NzParams.srcNdMatrixStride = self->ctx.tiling_->cinG * self->ctx.tiling_->dkHkWk; // src B stride (unit: element)
                                              ...
                                              const LocalTensor<typename Intf::SrcBT>& useB1Buf)
{
    ...
    dn2NzParams.srcDnMatrixStride = self->ctx.tiling_->dkHkWk; // src B stride (unit: element)
    dn2NzParams.dstNzMatrixStride = self->ctx.tiling_->c0;     // dst B stride (unit: element)

    dn2NzParams.dValue = curCoutSize;                                            // D: cout
    dn2NzParams.srcDValue = self->ctx.tiling_->cinG * self->ctx.tiling_->dkHkWk; // src D stride (unit: element)
                                                             ...
                                                             const LocalTensor<typename Intf::SrcBT>& useB1Buf)
{
    ...
    // DN (cout, cin, dk, hk, wk) -> NZ (cout1, splitHk, splitwk, cin1, cin0, cout0) = (cout1, splithkwk, cin_align,
    // cout0) B: cin, D: cout, N: hkwk DN (B, D, N) -> (D1, N, B, D0)
    ...
    dn2NzParams.dValue = curCoutSize;                              // D: cout
    dn2NzParams.srcDValue = cinOffset * self->ctx.tiling_->dkHkWk; // src D stride (unit: element)
    dn2NzParams.dstNzC0Stride = self->ctx.splitHkWkList_[self->ctx.splitIndex_] *
                                curCin1Cin0Size; // dst D stride (unit: 32B)
    ...
    for (uint32_t i = 0; i < self->ctx.splitHkList_[self->ctx.splitIndex_]; i++) {
        ...
        dstB1Offset += self->ctx.splitWkList_[self->ctx.splitIndex_] *
                       (dn2NzParams.dstNzNStride << self->ctx.tiling_->c0BitsB);
    }
}

template <class Intf>
__aicore__ inline void LoadToB1Dn2NzTransposeForKernelSplitHW(Intf* self, uint32_t curCinSize, uint32_t curCoutSize,
                                                              uint64_t out2B1SrcAddrOffset,
                                                              const LocalTensor<typename Intf::SrcBT>& useB1Buf)
{
    // DN (cout, cin, dk, hk, wk) -> NZ (cout1, splitHk, splitwk, cin1, cin0, cout0) = (cout1, splithkwk, cin_align,
    ...
    out2B1SrcAddrOffset += self->ctx.splitHIndex_ * self->ctx.tiling_->wk + self->ctx.splitWIndex_;
    Dn2NzParams dn2NzParams;
    dn2NzParams.dnNum = curCinSize;
    dn2NzParams.srcDnMatrixStride = self->ctx.dkHkWk_;
    dn2NzParams.dstNzMatrixStride = 1 << self->ctx.tiling_->c0BitsB;

    dn2NzParams.dValue = curCoutSize;
    dn2NzParams.srcDValue = self->ctx.tiling_->cinG * self->ctx.dkHkWk_;
    dn2NzParams.dstNzC0Stride = self->ctx.splitHkWkList_[self->ctx.splitIndex_] * curCin1Cin0Size;
    // 因为既要切H，又要切W，N维度只能取一个点
    dn2NzParams.nValue = 1;
    // 只搬运一个点，理论上dstNzNStride没有意义
    dn2NzParams.dstNzNStride = curCin1Cin0Size;

    uint64_t srcGmOffsetH = 0;
    uint32_t dstB1OffsetW = 0;
    uint32_t srcGmStride = self->ctx.tiling_->wk * self->ctx.tiling_->strideH;
    uint32_t dstB1Stride = curCin1Cin0Size << self->ctx.tiling_->c0BitsB;
    for (uint32_t hIdx = 0; hIdx < self->ctx.splitHkList_[self->ctx.splitIndex_]; hIdx++) {
        uint64_t srcGmOffsetW = srcGmOffsetH;
        for (uint32_t wIdx = 0; wIdx < self->ctx.splitWkList_[self->ctx.splitIndex_]; wIdx++) {
            DataCopy(useB1Buf[dstB1OffsetW], self->ctx.weightGlobal_[out2B1SrcAddrOffset + srcGmOffsetW], dn2NzParams);
            srcGmOffsetW += self->ctx.tiling_->strideW;
            dstB1OffsetW += dstB1Stride;
        }
        srcGmOffsetH += srcGmStride;
    }
}
                                                   ...
                                                   const LocalTensor<typename Intf::SrcBT>& useB1Buf)
{
    ...
    nd2NzParams.srcDValue = self->ctx.tiling_->cinG * self->ctx.tiling_->dkHkWk; // src N stride (unit: element)
                                              ...
                                              const LocalTensor<typename Intf::SrcBT>& useB1Buf)
{
    ...
    if (self->ctx.tiling_->enableVecTrans) {
        ...
        dn2NzParams.srcDValue = AlignUp16(self->ctx.tiling_->cinG) *
                                self->ctx.tiling_->dkHkWk; // Cout, Dk, Hk, Wk, Cin1, Cin0
    } else {
...
__aicore__ inline void LoadToB1Nd2NzForKernelSplit(Intf* self, uint32_t curCinSize, uint32_t curCoutSize,
                                                   ...
                                                   const LocalTensor<typename Intf::SrcBT>& useB1Buf)
{
    // ND (cout, dk, hk, wk, cin) -> NZ (cout1, splithk, splitwk, cin1, cin0, cout0) = (cout1, splithkwk, cin_align,
    // cout0) B: splitwk, D: cout, N: cin ND (B, N, D) -> (D1, N, B, D0)
    uint32_t curCin1Cin0Size = AlignUp16(curCinSize);
    if (curCin1Cin0Size != curCinSize) {
        InitZeroValue<Intf, typename Intf::SrcBT>(self, useB1Buf);
    }
                                              ...
                                              const LocalTensor<typename Intf::SrcBT>& useB1Buf)
{
    if constexpr (Intf::conv3dConfig.loadB2Condition == TPL_TRANSPOSE_ONLY) {
        ...
        if (self->ctx.tiling_->dkHkWk == 1) {
            ...
            } else if constexpr (Intf::conv3dConfig.kernelSplitMode == TPL_SPLIT_KERNEL_HW && !ksCoutFullLoad) {
                LoadToB1Dn2NzTransposeForKernelSplitHW(self, curCinSize, curCoutSize, out2B1SrcAddrOffset, useB1Buf);
            } else {
...
__aicore__ inline void LoadGmDataToB1(Intf* self, uint32_t kIdx, uint32_t curDkIdx)
{
    ...
    if (self->ctx.tiling_->enableVecTrans) {
        uint64_t out2B1SrcAddrOffset = static_cast<uint64_t>(curCoutIdx) * AlignUp16(self->ctx.tiling_->cinG) *
                                       ...
                                       curCinIdx +
                                       curDkIdx * self->ctx.tiling_->hkWk * AlignUp16(self->ctx.tiling_->cinG);
        ...
        if constexpr (Intf::Config::xType::format == Convolution3DBackprop::CubeFormat::NCDHW) {
            uint64_t out2B1SrcAddrOffset = static_cast<uint64_t>(curCoutIdx) * self->ctx.tiling_->cinG *
                                               self->ctx.tiling_->dkHkWk +
                                           curCinIdx * self->ctx.tiling_->dkHkWk + curDkIdx * self->ctx.tiling_->hkWk +
                                           self->ctx.curHkIdx_ * self->ctx.tiling_->wk + self->ctx.curWkIdx_;
            LoadGmDataToB1ForDn2Nz<Intf, ksCoutFullLoad>(self, curCinSize, curCoutSize, out2B1SrcAddrOffset, useB1Buf);
            ...
            uint64_t out2B1SrcAddrOffset = curCoutIdx + static_cast<uint64_t>(curCinIdx) * self->ctx.tiling_->cout +
                                           (curDkIdx * self->ctx.tiling_->hkWk +
                                            self->ctx.curHkIdx_ * self->ctx.tiling_->wk + self->ctx.curWkIdx_) *
            ...
            uint64_t out2B1SrcAddrOffset = static_cast<uint64_t>(curCoutIdx) * self->ctx.tiling_->hkWk *
                                               AlignUp16(self->ctx.tiling_->cinG) +
                                           (self->ctx.curHkIdx_ * self->ctx.tiling_->wk + self->ctx.curWkIdx_) *
                                               self->ctx.tiling_->cinG * self->ctx.tiling_->c0 +
