...
public:
    using CVQue = CVSyncQue<CVSyncQueConfig<PIPE_MTE1, PIPE_MTE3, PIPE_MTE3,
        CROSS_CORE_AIC2AIV_SEND_DN2NZ_FLAG,
        CROSS_CORE_AIV2AIC_RECV_DN2NZ_FLAG,
        PINGPONG_FREE_SLOTS, true> >;
...
private:
    __aicore__ static inline void InitDN2NZTiling(
        uint32_t h, uint32_t w, uint32_t& outBlockC0, uint32_t& outBlockHW)
    {
        uint32_t hw = h * w;
    ...
    __aicore__ inline void CopyInDN2NZ2UB(
        const GlobalTensor<T>& src,
        const LocalTensor<T>& l1,
        const LocalTensor<T>& ub,
        ...
        uint32_t nIdx, uint32_t cIdx, uint32_t hwIdx, uint32_t cLen, uint32_t hwLen)
    {
        ...
        uint64_t gmOffset = static_cast<uint64_t>(nIdx) * c * hw +
                            static_cast<uint64_t>(cIdx) * hw +
        ...
        CopyCbufToUbuf(
            ubAddr, l1Addr,
            1, hwLen * Ops::Base::CeilDiv(cLen, C0<T>()),
            0, 0);
    ...
    __aicore__ inline void CopyOutInAiv(
