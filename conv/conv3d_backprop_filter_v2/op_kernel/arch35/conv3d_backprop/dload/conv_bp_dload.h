...
//      完成全部块循环、装载、计算与块间同步
//   3. End 必须调用（第二十三轮 Drain→End）：消费跨块/跨半区背压残留 Set——漏调则残留 flag 污染同核
...
public:
                                ...
                                const DLoadConfig& config, uint32_t blockNum = 0)
    {
        config_ = config;
        blockNum_ = blockNum;
        // ★第二十三轮补（项 1）：fmap_/dy_ 成员删除——GM 地址即用即传（compute 类自持
        //   GlobalTensor，串联层无需保持副本）
    ...
    {
        // ★第二十三轮：AIC 判别——DLoad 为纯 Cube 模板（load3d/Mmad/fixpipe），AIV 核
        //   进入直接跳出（引擎 if constexpr 先例形态；Init/End 的标量侧操作对 AIV 无害，
        //   但 Process 的块循环不得在 AIV 上执行）
        if ASCEND_IS_AIV {
            return; // 纯 Cube 模板：AIV 核进入直接跳出（引擎 if constexpr 先例形态）
        }
