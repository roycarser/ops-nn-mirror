...
ge::graphStatus Conv3DBackpropFilterV2StreamKTiling::DoOpTiling()
{
    ...
    if (!MultiCoreSplitMN()) {
        ...
        ss << "the output shape info follows ncdhw format. "
           << "x shape is [" << runInfo_.batch << ", " << runInfo_.ci << ", " << runInfo_.di << ", " << runInfo_.hi
           << ", " << runInfo_.wi << "]. "
           << "output_backprop shape is [" << runInfo_.batch << ", " << runInfo_.co << ", " << runInfo_.dout << ", "
           << runInfo_.ho << ", " << runInfo_.wo << "]. "
           << "filter shape is [" << runInfo_.co << ", " << runInfo_.ci << ", " << runInfo_.kd << ", " << runInfo_.kh
           << ", " << runInfo_.kw << "]. "
        ...
        CUBE_INNER_ERR_REPORT(opName_,
                              "StreamK tiling template do optiling failed. Exceed L1 buffer size, please check the "
                              "shape and attribute. %s",
                              ss.str().c_str());
...
void Conv3DBackpropFilterV2StreamKTiling::AdjustSmallCaseBaseBlockForStreamK(uint64_t nBlockCnt)
{
    int64_t mCntMax = std::min(Ops::Base::CeilDiv(mmInfo_.mValue, static_cast<uint64_t>(BLOCK_CUBE)),
                               static_cast<uint64_t>(platformInfo_.core_num)) /
                      runInfo_.kd / runInfo_.real_g / nBlockCnt;
...
void Conv3DBackpropFilterV2StreamKTiling::AdjustSmallCaseBaseBlock()
{
    ...
    if ((platformInfo_.core_num % totalCnt == 0) &&
        ((static_cast<uint64_t>(runInfo_.batch * runInfo_.dout) >= splitCoreNum) ||
    ...
    if (totalCnt > (platformInfo_.core_num >> 1) && totalCnt < platformInfo_.core_num &&
...
bool Conv3DBackpropFilterV2StreamKTiling::IsSplitBatchDoutBetter()
{
    ...
    uint64_t singleShapeBatchDout = Ops::Base::CeilDiv(singleCoreBatchDout,
                                                       static_cast<uint64_t>(blockTiling_.coreStreamK));
...
void Conv3DBackpropFilterV2StreamKTiling::DoStreamKTiling()
{
    ...
    OP_LOGD(opName_, "Finish doing basic block stream k tiling, totalCnt[%lu], coreStreamK[%u].", totalCnt,
            blockTiling_.coreStreamK);
}
