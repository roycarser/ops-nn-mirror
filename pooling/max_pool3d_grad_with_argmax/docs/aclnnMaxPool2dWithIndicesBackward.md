...
每个算子分为[两段式接口](../../../docs/zh/context/two_phase_api.md)，必须先调用“aclnnMaxPool2dWithIndicesBackwardGetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnMaxPool2dWithIndicesBackward”接口执行计算。
...
- **参数说明：**
  ...
  - <term>Ascend 950PR/Ascend 950DT</term>：
    - gradOutput和self数据类型支持FLOAT、FLOAT16、BFLOAT16，数据格式支持ND、NCHW、NHWC。
  ...
  - <term>Atlas A2 训练系列产品/Atlas A2 推理系列产品</term>、<term>Atlas A3 训练系列产品/Atlas A3 推理系列产品</term>：
    - gradOutput和self数据类型支持FLOAT。
    - 数据格式支持ND和NCHW，输入3维时表示CHW取值。
    - indices数据类型支持INT32。仅支持dilation为（1，1）。
