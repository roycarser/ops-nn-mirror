...
/*!
 * \file avg_pool_grad_infershape.cpp
 * \brief
 */
...
static constexpr size_t ATTR_PADDING_POS = 2;
static constexpr size_t ATTR_FORMAT_POS = 3;
...
ge::graphStatus InferShape4AvgPoolGrad(gert::InferShapeContext* context)
{
    if (context == nullptr) {
        return GRAPH_FAILED;
    }
    OP_LOGD(context->GetNodeName(), "runtime2.0 AvgPoolGrad infershape running");
    ...
    auto padding = attrs->GetAttrPointer<char>(ATTR_PADDING_POS);
    OP_CHECK_NULL_WITH_CONTEXT(context, padding);
    OP_CHECK_IF(
        strcmp(padding, "SAME") != 0 && strcmp(padding, "VALID") != 0,
        OP_LOGE(context->GetNodeName(),"attr padding(%s) only support SAME、 VALID", padding), return GRAPH_FAILED);

    ge::graphStatus checkStatus = CheckKernelAndStrides(context, dataFormatStr);
    OP_CHECK_IF(checkStatus != ge::GRAPH_SUCCESS, OP_LOGD(context->GetNodeName(), "CheckKernelAndStrides failed"),
        return checkStatus);
    ...
    OP_LOGD(context->GetNodeName(), "runtime2.0 end AvgPoolGrad infershape");
    return ge::GRAPH_SUCCESS;
}

static ge::graphStatus InferDataType4AvgPoolGrad(gert::InferDataTypeContext* context)
{
    if (context == nullptr) {
        return GRAPH_FAILED;
    }
...
IMPL_OP_INFERSHAPE(AvgPoolGrad)
    .InputsDataDependency({0})
    .InferShape(InferShape4AvgPoolGrad)
    .InferDataType(InferDataType4AvgPoolGrad);
} // namespace ops