                        ...
                        const ge::Format& inputFormat)
{
    OP_CHECK_IF(
        ...
        OP_LOGE(context->GetNodeName(), "AvgPoolV2: input shape dim = %zu, should be equal 4",
                                        inputShape.GetDimNum()),
        return ge::GRAPH_FAILED);
    OP_CHECK_IF(
        outputShape.GetDimNum() != NCHW_DIMS,
        OP_LOGE(context->GetNodeName(), "AvgPoolV2: output shape dim = %zu, should be equal 4",
    ...
    OP_CHECK_IF(inputShape.GetShapeSize() <= 0,
                    OP_LOGE(context->GetNodeName(),
                                                    "AvgPoolV2: input shape size %ld less than zero failed",
                                                    inputShape.GetShapeSize()),
                    return ge::GRAPH_FAILED);

    OP_CHECK_IF(outputShape.GetShapeSize() <= 0,
                    OP_LOGE(context->GetNodeName(),
                                                    "AvgPoolV2: output shape size %ld less than zero failed",
    ...
    OP_CHECK_IF(
        ...
        OP_LOGE(context->GetNodeName(),
                                        "AvgPoolV2: the size of dim-n should be equal in inputShape and outShape, but get input [%ld], output [%ld]",
            inputShape.GetDim(nDim), outputShape.GetDim(nDim)),
        return ge::GRAPH_FAILED);
    ...
    OP_CHECK_IF(
        ...
        OP_LOGE(context->GetNodeName(),
                                        "AvgPoolV2: the size of dim-c should be equal in inputShape and outShape, but get input [%ld], output [%ld]",
                                        inputShape.GetDim(cDim), outputShape.GetDim(cDim)),
        return ge::GRAPH_FAILED);  
    return ge::GRAPH_SUCCESS;
}
...
static ge::graphStatus GetShapeAndDtype(gert::TilingContext* context, AvgPoolInputInfo& inputData, AvgPoolV2Common& commInfo)
{
    ...
    if (IsInvalidType(dtype)) {
        OP_LOGE(context->GetNodeName(), "AvgPoolV2: invalid dtype");
        return ge::GRAPH_FAILED;
    }
    ...
    OP_CHECK_IF(CheckShape(context, inputShape, outShape, inputData.inputFormat) != ge::GRAPH_SUCCESS,
                    OP_LOGE(context->GetNodeName(), "AvgPoolV2: check shape failed"),
                    return ge::GRAPH_FAILED);
    if (inputData.inputFormat == ge::Format::FORMAT_NCHW) {
        ...
        OP_LOGE(context->GetNodeName(),
                                        "AvgPoolV2: only support NCHW and NHWC, not support format.");
        return ge::GRAPH_FAILED;
    }
                                    ...
                                    AvgPoolInputInfo& inputData, const AvgPoolV2Common& commInfo)
{
    ...
    OP_CHECK_IF(strideDim != ONE_DIMS && strideDim != HW_DIMS && strideDim != NCHW_DIMS,
                    OP_LOGE(context, "AvgPoolV2: stride must have %d, %d, or %d elements ",
    ...
    OP_CHECK_IF(hStride <= 0 || wStride <= 0,
                    OP_LOGE(context->GetNodeName(),
                        "AvgPoolV2: The stride of the H and W dimensions should be greater than 0, not support [%ld, %ld]",
                         hStride, wStride),
                    return ge::GRAPH_FAILED); 
    return ge::GRAPH_SUCCESS;
}
                                ...
                                AvgPoolInputInfo& inputData, const AvgPoolV2Common& commInfo)
{
    ...
    OP_CHECK_IF(
        ...
        OP_LOGE(context, "AvgPoolV2: kernel_size must have %d, %d, or %d elements ", 
                                        ONE_DIMS, HW_DIMS, NCHW_DIMS),
        return ge::GRAPH_FAILED);
    ...
    OP_CHECK_IF(hKernelSize <= 0 || wKernelSize <= 0,
                    OP_LOGE(context->GetNodeName(),
                        "AvgPoolV2: The ksize of the H and W dimensions should be greater than 0, not support [%ld, %ld]",
                                  ...
                                  AvgPoolInputInfo& inputData, const AvgPoolV2Common& commInfo)
{
    if (commInfo.padModeStr == "VALID") {
        ...
        OP_CHECK_IF(padding->GetSize() != 4,
            OP_LOGE(context, "AvgPoolV2: pad list must have %d elements ", 4),
             return ge::GRAPH_FAILED);
        ...
        OP_CHECK_IF(topPad >= inputData.kernelSize[H_DIM] || bottomPad >= inputData.kernelSize[H_DIM],
                    OP_LOGE(context,
                        "AvgPoolV2: The topPad and bottomPad should be less than ksize of the H dimension, not support [%ld, %ld]",
                         topPad, bottomPad),
                    return ge::GRAPH_FAILED); 
        OP_CHECK_IF(leftPad >= inputData.kernelSize[W_DIM] || rightPad >= inputData.kernelSize[W_DIM],
                    OP_LOGE(context,
                        "AvgPoolV2: The leftPad and rightPad should be less than ksize of the W dimension, not support [%ld, %ld]",
                         leftPad, rightPad),
                    return ge::GRAPH_FAILED);
        inputData.pad[TOP_PAD_INDEX] = topPad;
        inputData.pad[BOTTOM_PAD_INDEX] = bottomPad;
        ...
        OP_LOGE(context, "AvgPoolV2: not support padmode %s", commInfo.padModeStr.c_str());
        return ge::GRAPH_FAILED;
    }
    return ge::GRAPH_SUCCESS;
}
                            ...
                            AvgPoolInputInfo& inputData, AvgPoolV2Common& commInfo)
{
    ...
    if (inputFormatStr == "NCHW") {
        ...
        OP_LOGE(context, 
            "AvgPoolV2: only support NCHW and NHWC, not support format %s",
...
static ge::graphStatus CheckOutPutShapeForValid(gert::TilingContext* context, AvgPoolInputInfo& inputData)
{
    ...
    if (inputData.outShape[H_DIM] != expectedH || inputData.outShape[W_DIM] != expectedW) {
        OP_LOGE(context,
                                        "AvgPoolV2: when padmode is VALID, the outputshape in \
...
static ge::graphStatus CheckOutPutShapeForSame(gert::TilingContext* context, AvgPoolInputInfo& inputData)
{
    ...
    if (inputData.outShape[H_DIM] != expectedH || inputData.outShape[W_DIM] != expectedW) {
        OP_LOGE(context,
                                        "AvgPoolV2: when padmode is SAME, the outputshape in \
h-dim and w-dim should be [%ld] [%ld], but got [%ld] [%ld]",
                                        expectedH, expectedW, inputData.outShape[H_DIM],
                                        inputData.outShape[W_DIM]);
        return ge::GRAPH_FAILED;
...
static ge::graphStatus CheckOutPutShapeForCalculated(gert::TilingContext* context, AvgPoolInputInfo& inputData)
{
    ...
    if (inputData.outShape[H_DIM] != expectedH || inputData.outShape[W_DIM] != expectedW) {
        OP_LOGE(context,
                                        "AvgPoolV2: when padmode is Calculated, the outputshape in \
h-dim and w-dim should be [%ld] [%ld], but got [%ld] [%ld]",
                                        expectedH, expectedW, inputData.outShape[H_DIM],
                                        inputData.outShape[W_DIM]);
        return ge::GRAPH_FAILED;
                                                 ...
                                                 const AvgPoolV2Common& commInfo)
{
    ...
    OP_LOGE(context, 
                                    "AvgPoolV2: unsupported pad mode [%s], only VALID, SAME and CALCULATED are supported", 
...
ge::graphStatus GetAvgPoolV2ShapeAttrsInfo(gert::TilingContext* context, AvgPoolInputInfo& inputData)
{
    ...
    if (inputData.globalPooling) {
        OP_CHECK_IF((inputData.inputShape[0] != 0 && inputData.outShape[0] != 1) ||
                        (inputData.inputShape[1] != 0 && inputData.outShape[1] != 1),
                        OP_LOGE(context,
        "AvgPoolV2: when global_pooling is true, the outputshape in h-dim and w-dim must be 1 if the size \
of corresponding inputshape is not zero"), return ge::GRAPH_FAILED);
        inputData.pad[TOP_PAD_INDEX] = 0;
        inputData.pad[BOTTOM_PAD_INDEX] = 0;
        inputData.pad[LEFT_PAD_INDEX] = 0;
        inputData.pad[RIGHT_PAD_INDEX] = 0;
