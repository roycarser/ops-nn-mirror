...
class SituGluGrad : public OpDef {
public:
    explicit SituGluGrad(const char* name) : OpDef(name)
    {
        this->Input("grad_y")
            .ParamType(REQUIRED)
            .DataType(xDtype)
        ...
        this->Input("x")
            ...
            .UnknownShapeFormat(xUnknownFormat)
            .AutoContiguous();
        this->Output("grad_x").ParamType(REQUIRED).DataType(xDtype).Format(xFormat).UnknownShapeFormat(xUnknownFormat);
