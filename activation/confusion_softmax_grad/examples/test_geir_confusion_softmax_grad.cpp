...
/*!
 * \file test_geir_confusion_softmax_grad.cpp
 * \brief
 */
    ...
    Graph& graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto confusionSoftmaxGrad_1 = op::ConfusionSoftmaxGrad("confusionSoftmaxGrad_1");
    std::vector<int64_t> xShape = {2, 3, 4, 5};
    ADD_INPUT(1, grad, inDtype, xShape);
    ADD_INPUT(2, x, inDtype, xShape);
    ADD_OUTPUT(1, y, inDtype, xShape);
    outputs.push_back(confusionSoftmaxGrad_1);
    // 添加完毕
    return SUCCESS;
}
