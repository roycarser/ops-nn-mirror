...
- 接口功能：SiTU门控线性单元（SiTU Gated Linear Unit）激活函数。对输入张量x沿指定维度切分为门控（gate）与上路径（up）两半，按SiTU公式计算输出。
- 计算公式：

  对给定的输入张量x，其维度为[a, b, c, d, e, f, g, ...]，aclnnSituGlu对其执行以下计算：

  1. 将x基于输入参数dim进行合轴，合轴后维度为[pre, cut]。其中cut轴为需要切分为两个张量的轴，cut必须为偶数。令h = cut // 2。

  2. 根据输入参数activateLeft对x进行前后切分：
  ...
  3. 根据输入参数beta、linearBeta进行SiTU计算：
...
每个算子分为[两段式接口](../../../docs/zh/context/two_phase_api.md)，必须先调用"aclnnSituGluGetWorkspaceSize"接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用"aclnnSituGlu"接口执行计算。
...
- **返回值**
  ...
  <tbody>
    ...
    <tr>
      <td>x或out的shape维度不在支持的范围之内。</td>
    </tr>
    <tr>
      <td>dim不在指定的取值范围内或x在dim对应维度上不能被2整除。</td>
    </tr>
    <tr>
      <td>out的维度数与x不一致，或out在dim对应维度上的大小不等于x的一半。</td>
    </tr>
    <tr>
      <td>out的数据类型与x不匹配。</td>
    </tr>
