...
#define ADD_INPUT(intputIndex, intputName, intputDtype, inputShape)                                                 \
    ...
    ret = GenConstData(placeholder##intputIndex##_shape, tensor_placeholder##intputIndex,                           \
...
int32_t GenConstData(vector<int64_t> shapes, Tensor& input_tensor, TensorDesc& input_tensor_desc, DataType data_type,
                     float value)
{
...
int32_t WriteDataToFile(string bin_file, uint64_t data_size, uint8_t* inputData)
{
    ...
    fclose(fp);
    return (written == data_size) ? SUCCESS : FAILED;
}
                     ...
                     std::vector<Operator>& outputs, Graph& graph)
{
    ...
    // Create FusedBiasLeakyRelu operator
    auto add1 = op::FusedBiasLeakyRelu("fused_bias_leaky_relu_0");

    // Set attributes
    add1.set_attr_negative_slope(0.2f);
    add1.set_attr_scale(1.414213562373f);

    // x: (2, 4)
    std::vector<int64_t> tensorShape = {2, 4};
    ADD_INPUT(1, x, inDtype, tensorShape);

    // bias: (2, 4) -- same shape as x
    ADD_INPUT(2, bias, inDtype, tensorShape);

    // y output: (2, 4)
    ADD_OUTPUT(1, y, inDtype, tensorShape);
