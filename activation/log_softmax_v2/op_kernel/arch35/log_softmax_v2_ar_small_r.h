...
#ifndef LOG_SOFTMAX_V2_AR_SMALL_R_H
#define LOG_SOFTMAX_V2_AR_SMALL_R_H
...
class LogSoftmaxV2ArSmallR : public SoftmaxV2OpsBase {
...
public:
    __aicore__ inline LogSoftmaxV2ArSmallR(TPipe* pipe) { pipe_ = pipe; };
    ...
    __aicore__ inline void Init(GM_ADDR x, GM_ADDR y, const SoftmaxV2ArSmallRTilingData* tilingDataIn)
    {
        ...
        pipe_->InitBuffer(xSumBuffer_, tl_->tileA0Len * sizeof(float));
    }
    ...
    __aicore__ inline void Process()
    {
        ...
        sumLocal_ = xSumBuffer_.Get<float>();
...
private:
    ...
    __aicore__ inline void CalcOutput(uint32_t curTileA0Len, uint32_t totalRLen)
    {
        ...
        __VEC_SCOPE__
        {
            ...
            MicroAPI::RegTensor<float> sumReg, lnReg;
            MicroAPI::MaskReg mask;
    ...
    {
        int rRepeartTimes = Ops::Base::CeilDiv(static_cast<int64_t>(rAligned), DATA_BLOCK_COUNT_HALF);
        for (int i = 0; i < rRepeartTimes; i++) {
            ...
            uint32_t aRepeartTimes = Ops::Base::CeilDiv(curTileA0Len, uint32_t(DATA_BLOCK_COUNT));
    ...
    {
        int rRepeartTimes = Ops::Base::CeilDiv(static_cast<int64_t>(rAligned), tl_->rTileBase);
        for (int i = 0; i < rRepeartTimes; i++) {
            TransDataTo5HDParams params;
            int64_t aRepeartTimes = Ops::Base::CeilDiv(static_cast<int64_t>(curTileA0Len), tl_->rTileBase);
            params.repeatTimes = aRepeartTimes;
            ...
            for (int j = 0; j < DATA_BLOCK_COUNT; j++) {
                uint32_t offset = tl_->rTileBase * i + rAligned * j; // R和16对齐，即16*2=32B对齐
