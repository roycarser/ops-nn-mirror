...
public:
    ...
    __aicore__ inline SoftmaxV2ARA(const SoftmaxV2ARATilingData* tilingDataIn) { tilingData_ = tilingDataIn; }
    ...
    __aicore__ inline void SumRLessThan4(__ubuf__ float* xTmpLocal, __ubuf__ float* xReduceLocal, uint32_t curTileA0Len)
    {
        ...
        __VEC_SCOPE__
        {
            ...
            for (uint16_t k = 0; k < aLoopCount; k++) {
                ...
                StoreAlign(((__ubuf__ float*)xTmpLocal + validNumInXUb + aLoopOffset), zero, pregLoop);
