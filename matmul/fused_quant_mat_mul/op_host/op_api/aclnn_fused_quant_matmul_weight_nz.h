 ...
 */
#ifndef OP_API_INC_FUSED_QUANT_MATMUL_NZ
#define OP_API_INC_FUSED_QUANT_MATMUL_NZ
...
/**
 * @brief aclnnFusedQuantMatmulWeightNz的第一段接口，根据具体的计算流程，计算workspace大小。
 ...
 */
ACLNN_API aclnnStatus aclnnFusedQuantMatmulWeightNzGetWorkspaceSize(const aclTensor *x1, const aclTensor *x2,
...
/**
 * @brief aclnnFusedQuantMatmulWeightNz的第二段接口，用于执行计算。
 * @param [in] workspace: 在npu device侧申请的workspace内存起址。
 * @param [in] workspace_size: 在npu device侧申请的workspace大小，由第一段接口aclnnFusedQuantMatmulWeightNzGetWorkspaceSize获取。
 * @param [in] exector: op执行器，包含了算子计算流程。
 * @param [in] stream: acl stream流。
 * @return aclnnStatus: 返回状态码
 */
ACLNN_API aclnnStatus aclnnFusedQuantMatmulWeightNz(void* workspace, uint64_t workspaceSize, aclOpExecutor* executor,
                                         aclrtStream stream);
...
#endif  // OP_API_INC_FUSED_QUANT_MATMUL_NZ
