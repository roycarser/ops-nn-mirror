...
每个算子分为[两段式接口](../../../docs/zh/context/two_phase_api.md)，必须先调用“aclnnBatchMatMulWeightNzGetWorkspaceSize”接口获取入参并根据流程计算所需workspace大小，再调用“aclnnBatchMatMulWeightNz”接口执行计算。
...
- **参数说明：**
  ...
  <tbody>
    <tr>
      ...
      <li>self的第一个维度b需要与mat2第一个维度b满足<a href="../../../docs/zh/context/broadcast_relationship.md">broadcast关系</a>。</li>
      </ul></td>
    ...
    <tr>
      ...
      <td><ul><li>数据类型需要与self满足数据类型推导规则（参见<a href="../../../docs/zh/context/deduction_relationship.md">互推导关系</a>和<a href="#约束说明">约束说明</a>）。</li><li>mat2的Reduce维度需要与self的Reduce维度大小相等。</li>
      ...
      <li>mat2的第一个维度b需要与self第一个维度b满足<a href="../../../docs/zh/context/broadcast_relationship.md">broadcast关系</a>。</li>
      </ul></td>
      <td>BFLOAT16、FLOAT16</td>
      <td>NZ</td>
      <td>5</td>
    ...
    <tr>
      ...
      <td><ul><li>数据类型需要与self与mat2推导之后的数据类型满足推导规则（参见<a href="../../../docs/zh/context/deduction_relationship.md">互推导关系</a>和<a href="#约束说明">约束说明</a>）。</li>
...
self和mat2数据类型为float16，mat2为NZ格式场景下的示例代码如下，仅供参考，具体编译和执行过程请参考[编译与运行样例](../../../docs/zh/context/compile_and_run_sample.md)。
  ...
  int main() {
    ...
    std::vector<uint16_t> selfHostData(1024, 0x3C00); // float16_t用0x3C00表示int_16的1
    std::vector<uint16_t> mat2HostData(1024, 0x3C00); // float16_t用0x3C00表示int_16的1
