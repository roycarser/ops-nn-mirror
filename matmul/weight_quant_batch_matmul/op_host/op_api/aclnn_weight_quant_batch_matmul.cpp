...
inline static bool SelectFixPipe(int64_t M) { return M <= M_THRESHOLD; }

// bias can be nullptr
inline static bool CheckNotNull(const aclTensor* x1, const aclTensor* x2, const aclTensor* addOffset,
                                const aclTensor* mulScale, const aclTensor* diagonalMatrix, const aclTensor* deqOffset,
...
static bool IsFloatEqual(float f1, float f2) { return std::abs(f1 - f2) <= std::numeric_limits<float>::epsilon(); }
...
static bool CheckAntiQuantTensorValid(const aclTensor* addOffset, const aclTensor* mulScale,
                                      const aclTensor* diagonalMatrix, const aclTensor* deqOffset,
                                      const aclTensor* deqScale, bool transposeX2, int64_t M, int64_t N)
{
    if (SelectFixPipe(M)) {
        if (!CheckOffsetAndScale(deqOffset, false, N)) {
            OP_LOGE(ACLNN_ERR_PARAM_INVALID,
                    "fixpipe Offset's broad cast dim is not equal to one or N, can't broadcast");
            return false;
        }
        if (!CheckOffsetAndScale(deqScale, false, N)) {
            OP_LOGE(ACLNN_ERR_PARAM_INVALID,
                    "fixpipe Scale's broad cast dim is not equal to one or N, can't broadcast");
            return false;
        }
        ...
        if ((addOffset != nullptr) && !CheckOffsetAndScale(addOffset, transposeX2, N)) {
            OP_LOGE(ACLNN_ERR_PARAM_INVALID,
                    "single op Offset's broad cast dim is not equal to one or N, can't broadcast");
            return false;
        }
        if ((mulScale != nullptr) && !CheckOffsetAndScale(mulScale, transposeX2, N)) {
            OP_LOGE(ACLNN_ERR_PARAM_INVALID,
                    "single op Scale's broad cast dim is not equal to one or N, can't broadcast");
            return false;
        }
    }
    return true;
...
static bool CheckShapeValid(const aclTensor* x1, const aclTensor* x2, const aclTensor* bias, const aclTensor* addOffset,
                            const aclTensor* mulScale, const aclTensor* diagonalMatrix, const aclTensor* deqOffset,
                            const aclTensor* deqScale, const aclTensor* out, bool transposeX1, bool transposeX2)
{
    ...
    if ((dimTensor1 < minDim) || (dimTensor2 < minDim)) {
        OP_LOGE(ACLNN_ERR_PARAM_INVALID,
                "The dims of the two inputs should be same or larger than 2,"
                "now the shapes are  %s, %s",
                op::ToString(x1Shape).GetString(), op::ToString(x2Shape).GetString());
        return false;
    }

    int64_t M = transposeX1 ? x1->GetViewShape().GetDim(dimTensor1 - 1) : x1->GetViewShape().GetDim(dimTensor1 - 2);
    int64_t x1KDim = transposeX1 ? x1->GetViewShape().GetDim(dimTensor1 - 2) :
                                   x1->GetViewShape().GetDim(dimTensor1 - 1);
    int64_t x2KDim = transposeX2 ? x2->GetViewShape().GetDim(dimTensor2 - 1) :
                                   x2->GetViewShape().GetDim(dimTensor2 - 2);
    ...
    if (!CheckAntiQuantTensorValid(addOffset, mulScale, diagonalMatrix, deqOffset, deqScale, transposeX2, M, N)) {
        return false;
    }
...
{
    // 1. 检查参数是否为空指针
    CHECK_RET(CheckNotNull(x1, x2, addOffset, mulScale, diagonalMatrix, deqOffset, deqScale, out, transposeX1),
              ACLNN_ERR_INNER_NULLPTR);
    // 2. 检查输入的数据类型是否在API支持的数据类型范围之内，需要根据api定义校验
    CHECK_RET(CheckDtypeValid(x1, x2, bias, addOffset, mulScale, diagonalMatrix, deqOffset, deqScale, out, transposeX1),
              ACLNN_ERR_PARAM_INVALID);
    // 3. 检查Shape是否支持
    CHECK_RET(CheckShapeValid(x1, x2, bias, addOffset, mulScale, diagonalMatrix, deqOffset, deqScale, out, transposeX1,
                              transposeX2),
              ACLNN_ERR_PARAM_INVALID);
...
const aclTensor* WeightQuantMatmulSingleOp(const aclTensor* x1, const aclTensor* x2, const aclTensor* addOffset,
                                           const aclTensor* mulScale, const aclTensor* bias, float antiquantScale,
                                           float antiquantOffset, bool transposeX1, bool transposeX2, aclTensor* out,
                                           aclOpExecutor* executor)
{
    ...
    if ((x1->GetViewShape().GetDimNum() > 2) || (x2->GetViewShape().GetDimNum() > 2)) {
        matmulOut = Ops::NN::ExecBatchMatmulOpWithBiasAndAttrs(x1, antiquantX2, bias, out, transposeX1, transposeX2, 0,
                                                               executor);
    } else {
        // 先直调l0 完成功能
        matmulOut = l0op::MatMulNd(x1, antiquantX2, bias, nullptr, transposeX1, transposeX2, 0, 0, executor);
    }
    return matmulOut;
}
...
aclnnStatus aclnnWeightQuantBatchMatmulGetWorkspaceSize(const aclTensor* x1, const aclTensor* x2,
                                                        ...
                                                        bool transposeX1, bool transposeX2, float antiquantScale,
                                                        float antiquantOffset, aclTensor* out, uint64_t* workspaceSize,
                                                        aclOpExecutor** executor)
{
    ...
    // 入参检查
    if (CheckParam(x1, x2, bias, addOffset, mulScale, diagonalMatrix, deqOffset, deqScale, out, transposeX1,
                   transposeX2) != ACLNN_SUCCESS) {
    ...
    if (SelectFixPipe(m)) {
        ...
        matmulOut = l0op::WeightQuantBatchmatmul(x1, x2, bias, diagonalMatrix, deqOffset, reformatDeqScale, transposeX1,
                                                 transposeX2, unique_executor.get());
    } else {
